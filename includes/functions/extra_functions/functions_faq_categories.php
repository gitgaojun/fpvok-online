  ...
  ///////////////////Derek Buntin Edited
   function zen_get_faq_category_tree($faq_parent_id = '0', $spacing = '', $exclude = '', $faq_category_tree_array = '', $include_itself = false, $faq_category_has_products = false, $limit = false) {
	...
	function zen_get_faq_in_category($cat_id){
		...
		$faq_in_category_sql = "SELECT f.`faqs_id`, fd.`faqs_name`,ft.faq_categories_id FROM faqs f, faqs_description fd, faqs_to_faq_categories ft WHERE fd.faqs_id=f.faqs_id AND ft.faqs_id=f.faqs_id AND f.`faqs_status` = 1 AND faq_categories_id = ". $cat_id;
		...
		if ($faq_in_category->RecordCount()>0){
			while (!$faq_in_category->EOF){
				...
				$faq_in_category_String .= '<li><a '.$class.' href="'.zen_href_link(zen_get_info_faq_page($faq_in_category->fields['faqs_id']),'fcPath=' . $faq_in_category->fields['faq_categories_id'] . '&faqs_id=' . $faq_in_category->fields['faqs_id']).'" >'.$faq_in_category->fields['faqs_name'].'</a></li>';
