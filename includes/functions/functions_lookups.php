 ...
 */
  function zen_get_country_name($country_id) {
    ...
    function zen_get_product_shijian($product_id) { 
    global $db;
    $product = $db->Execute("select specials_date_available,expires_date
                             from " . TABLE_SPECIALS . "
                             where products_id = '" . (int)$product_id . "'");
	if ($product->RecordCount() > 0){
    list($year,$month,$day)=split ("-",$product->fields['expires_date']);
	$second=mktime(0,0,0,$month,$day,$year);
    return $second;
	}else{
	return false;
	}
 ...
 */
  ...
  function zen_get_products_weight($product_id) {
    ...
    function zen_get_diggid_unit($product_id) { 
 
    global $db;
    $product = $db->Execute("select products_diggs 
                             from " . TABLE_PRODUCTS . "
                             where products_id = '" . (int)$product_id . "'");
    return $product->fields['products_diggs'];
  }
/**
...
*/
  function zen_get_products_name($product_id, $language = '') {
    ...
    function zen_get_products_model($product_id, $language = '') {
    global $db;
  ...
  function zen_get_products_count($product_id) {
    global $db;
 ...
 */
  function zen_get_categories_image($what_am_i) {
    ...
    function zen_get_categories_pant_id($what_am_i) {
    global $db;

    $the_categories_image_query= "select parent_id from " . TABLE_CATEGORIES . " where categories_id= '" . $what_am_i . "'";
    $the_products_category = $db->Execute($the_categories_image_query);

    return $the_products_category->fields['parent_id'];
  }

    function zen_get_product_price_tel($who_am_i) {
    global $db;
    $the_categories_name_query= "select cd.categories_name,cd.categories_id from " . TABLE_CATEGORIES_DESCRIPTION . " cd, " . TABLE_PRODUCTS . " p where cd.categories_id= p.master_categories_id and p.products_id = '" . $who_am_i . "' and cd.language_id= '" . $_SESSION['languages_id'] . "'";

    $the_categories_name = $db->Execute($the_categories_name_query);

    return 'in <a class="ViewLarger" href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' .$the_categories_name->fields['categories_id']) . '">'.$the_categories_name->fields['categories_name'].'</a>';
  }
/*
 ...
 */
  ...
  function zen_get_discount_count($current_category_id) {
    global $db;
	...
	$product_in_categories_sql = '( p.master_categories_id ='.$product_in_categories_sql.')';			
		...
		$product_in_categories_sql = ' 1=1';
	}
    $values_values = $db->Execute("select p.products_id
                                   from products p,specials s
                                   where p.products_id = s.products_id and s.status =1 and ".$product_in_categories_sql);

    return $values_values->RecordCount();
  }
/*
 ...
 */
  function zen_get_products_image($product_id, $width = SMALL_IMAGE_WIDTH, $height = SMALL_IMAGE_HEIGHT) {
    ...
    return zen_image(DIR_WS_IMAGES . $look_up->fields['products_image'], SEO_COMMON_KEYWORDS . ' ' .zen_get_products_name($product_id), $width, $height);
  }
...
WHERE c.`categories_id` = ".$categories_id;
  ...
  function zen_get_faq_caname($categories_id){
  	global $db;
  	$sql = "SELECT faq_categories_name FROM faq_categories_description
WHERE faq_categories_id = ".(int)$categories_id;
  	$sqlArray = $db->Execute($sql);
  	return $sqlArray->fields['faq_categories_name'];
  }  
...
WHERE c.`categories_id` = ".$categories_id;
  ...
  function zen_get_reviews($products_id){
      ...
      $outString = '<span class="commnet_write"><img src="images/stars5.gif" align="absmiddle">&nbsp;&nbsp;('.sizeof($reviewArray).')&nbsp;&nbsp;<a href="' . zen_href_link(zen_get_info_page($products_id), 'cPath='.zen_get_generated_category_path_rev($_GET['cPath']). '&products_id=' .$products_id) . '#review" >Reviews</a></span>';
  	}else{
  		$outString = '<span class="commnet_write"><img src="images/stars5.gif" align="absmiddle">&nbsp;&nbsp;('.sizeof($reviewArray).')&nbsp;&nbsp;<a href="' . zen_href_link(zen_get_info_page($products_id), 'cPath='.zen_get_generated_category_path_rev($_GET['cPath']). '&products_id=' .$products_id) . '#review" >Reviews</a></span>';
  	}
  	return $outString;
  ...
  function zen_get_reviewst($products_id){
  	global $db;
  	$reviewArray = array();
  	$querySql = "SELECT r.`reviews_rating` FROM reviews r
                 WHERE r.`status` = 1 AND r.`products_id` = ".$products_id."
      ...
      $outString .= '<a href="' . zen_href_link(zen_get_info_page($products_id), 'cPath='.zen_get_generated_category_path_rev($_GET['cPath']). '&products_id=' .$products_id) . '" >'.sizeof($reviewArray).' reviews</a></span>';
  	}else{
  		$outString = '<span class="commnet_write"><a href="' . zen_href_link(zen_get_info_page($products_id), 'cPath='.zen_get_generated_category_path_rev($_GET['cPath']). '&products_id=' .$products_id) . '" >'.sizeof($reviewArray).' reviews</a></span>';
  	}
  	return $outString;
  }
/*
...
/*
* Return sidebox status
*
*/
   function zen_get_layout_boxes_status($boxName){
   	global $db;
   	$query = "SELECT layout_box_status FROM layout_boxes WHERE layout_box_name='".$boxName."'";
   	$layout_boxes = $db->Execute($query);
    if ($layout_boxes->RecordCount() > 0){
    	return $layout_boxes->fields['layout_box_status'];
    }else {
    	return 0;
    }
   }
   function zen_get_banners_heard($boxName){
   	global $db;
	$str_banners = "";
	$i=1;
   	$query = "SELECT banners_title,banners_url,banners_id,banners_image,banners_open_new_windows FROM banners WHERE banners_group ='".$boxName."' and status =1 order by banners_sort_order asc";
   	$layout_boxes = $db->Execute($query);
    if ($layout_boxes->RecordCount() > 0){
	while(!$layout_boxes->EOF){
	if ($layout_boxes->fields['banners_url'] ==1){
	$strmk = 'target="_blank"';
	    }else{
	$strmk = '';	
		}
	$str_clas = ($i==1?' ':' class="qina"');
    	$str_banners .= '<div '.$str_clas.' align="center"><a href="/'.$layout_boxes->fields['banners_url'].' '.$strmk.'" ><img alt="'.$layout_boxes->fields['banners_title'].'" src="images/'.$layout_boxes->fields['banners_image'].'" align="right" style="margin-top:10px"></a></div>';
		$i++;
