    ...
    if ($display_special_price) {
      ...
      if ($display_sale_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_normal_price = '<strong class="red">'.$currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])).'</strong>';
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
...
function zen_get_products_price_accessories($products_id) {
    global $db;
    $product_check = $db->Execute("select products_price_dapei from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
	
    return number_format($product_check->fields['products_price_dapei'],2);
  }
