...
// Parse search string into indivual objects
  function zen_parse_search_string($search_str = '', &$objects,$set_operator='') {
...
// add default logical operators if needed
    ...
    for($i=0; $i< $objectsNum ; $i++) {
      ...
      if ( ($objects[$i] != 'and') &&
           ...
           ($objects[$i+1] != 'or') &&
           ($objects[$i+1] != ')') ) {
           	if (isset($set_operator)&&$set_operator!=ADVANCED_SEARCH_DEFAULT_OPERATOR)
           	{
           		$temp[] = $set_operator;
           	}
           	else 
 ...
 */
  function zen_js_zone_list($country, $form, $field) {
    ...
    while (!$countries->EOF) {
      ...
      $countries->MoveNext();
	  if($field=='b_zone_id'){
      $output_string .= '    show("zone_id_tr2");hide("zone_id_tr3");theForm.state.value="";' . "\n" ;
	  }else{
	  ...
	  if($field=='b_zone_id'){
    $output_string .= '  } else {' . "\n" .
                      '    hide("zone_id_tr2");show("zone_id_tr3");' . "\n" .
                      '  }' . "\n";
	  }else{
    $output_string .= '  } else {' . "\n" .
