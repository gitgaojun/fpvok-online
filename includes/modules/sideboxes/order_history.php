...
//		print_r($orders_history_query);
  ...
  $products_history_query = "select distinct op.products_id,o.customers_state,o.customers_country,opp.products_name 
                   from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_PRODUCTS_DESCRIPTION . " opp, " . TABLE_PRODUCTS . " p
                   where o.orders_id = op.orders_id
                   and op.products_id = p.products_id
				   and p.products_id = opp.products_id 
                   and p.products_status = '1'
                   group by products_id
                   order by o.date_purchased desc
                   limit 10";
      ...
      while (!$products_history->EOF) {
        ...
        $customer_orders[$rows]['name'] = strlen($products_history->fields['products_name'])>50? substr($products_history->fields['products_name'],0,50).'...':$products_history->fields['products_name'];
