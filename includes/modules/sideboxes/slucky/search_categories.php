...
// don't build a tree when no categories
    if(isset($list_box_contents)) reset($list_box_contents);
    $totalNum = sizeof($list_box_contents);
    for ($i = 0;$i < $totalNum; $i++){
    	$priceListArray[] = $list_box_contents[$i]['products_price'];
    }
    sort($priceListArray);
    ...
    for ($i=0; $i<$priceListRowNum; $i++){
     echo '<li>';
     echo '<a href="'.cleanSameArg2('min_price','max_price').'&min_price='.floatval($priceListArray[$totalNum*$i/$priceListRowNum]).'&max_price='.floatval($priceListArray[$totalNum*($i+1)/$priceListRowNum-1]).'">';
