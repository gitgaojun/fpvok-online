...
<?php
 ...
 if ($recently_sold->RecordCount()>0){
 	  ...
 	  echo '<div id="recently_sold_items" style="overflow: hidden; height: 200px;">';
 	  echo '<ul id="recently_sold_items_a" class="top_selling pad_10px pad_t" >';
 	  while (!$recently_sold->EOF){
      echo '<li class="clear"><a href="'.zen_href_link(zen_get_info_page($recently_sold->fields['products_id']),'products_id='.$recently_sold->fields['products_id']).'">'.zen_image_OLD(DIR_WS_IMAGES . $recently_sold->fields['products_image'],SEO_COMMON_KEYWORDS . ' ' .$recently_sold->fields['products_name'],42,42,' class="fl"').'</a>';
