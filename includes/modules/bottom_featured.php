...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  $featured_products_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id
                           ...
                           and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' limit 20";
} else {
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $featured_products_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id
                                ...
                                and p.products_id in (" . $list_of_products . ")  limit 20";
  }
}
...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
    ...
    'text' => (($featured_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '"  class="ih4" >' . zen_image_OLD(DIR_WS_IMAGES .str_replace("s/","l/", $featured_products->fields['products_image']), SEO_COMMON_KEYWORDS . ' ' .$featured_products->fields['products_name'], 165, 165) . ' </a>') . '<br /><span class="line_120" ><a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' .(strlen($products_name)>50 ? substr($products_name,0,50).'...':$products_name)   .'</a></span>'.'<p class="b red">' . $products_price .' </p>'.zen_get_reviews($featured_products->fields['products_id']));
