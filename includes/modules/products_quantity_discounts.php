...
// retrieve the list of discount levels for this product
  ...
  if ($products_discounts_query->RecordCount() <=0 ) {
	...
	$products_cat = $db->Execute("SELECT categories_discount_type FROM " . TABLE_CATEGORIES ." WHERE categories_id = " . (int)$current_category_id );
...
$columnCount = 0;
if ($type_cat==1){
$products_discount_type = $products_cat->fields['categories_discount_type'];
}else{
...
while (!$products_discounts_query->EOF) {
  ...
  $quantityDiscounts[$columnCount]['show_qty2'] = number_format($products_discounts_query->fields['discount_qty']);
