...
if ($num_categories > 0) {
  ...
  while (!$featured_categories->EOF and !($row == 2)) {
	...
	$cPath_new2 = str_replace('=0_', '=', $cPath_new);
...
WHERE c1.categories_id=c.categories_id and parent_id = '.$featured_categories->fields['categories_id'] .' and c.language_id = '. (int)$_SESSION['languages_id'] . ' and c1.categories_status=1 order by sort_order asc limit 5 ';
		...
		$subCatoryString .= '</dl><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '" class="more_product">More wholesale products ></a>';
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<div><a class="ih4" href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '" title="'.$featured_categories->fields['categories_name'].'">' . zen_image(DIR_WS_IMAGES . $featured_categories->fields['categories_image'],$featured_categories->fields['categories_name'], 165, 128) . '</a></div><h3 class="margin_t line_120 pad_b2"><a class="uu9" href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '" title="'.$featured_categories->fields['categories_name'].'">'. $featured_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString);
