  ...
  function collect_posts() {
    ...
    if ($_SESSION['cc_id'] > 0) {
      ...
      if (!$foundvalid) {
        ...
        $this->clear_posts();
        if (!$foundvalid) zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL',true, false));
      }
    }
    ...
    if ((isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] != '') || (isset($discount_coupon->fields['coupon_code']) && $discount_coupon->fields['coupon_code'] != '')) {
      ...
      if ($coupon_result->fields['coupon_type'] != 'G') {
        ...
        if (!$foundvalid) zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, 'credit_class_error_code=' . $this->code . '&credit_class_error=' . urlencode(TEXT_INVALID_COUPON_PRODUCT . ' ' . $dc_check), 'SSL',true, false));
      ...
      if (($coupon->RecordCount() > 0 && $order_total !=0) || ($coupon->RecordCount() > 0 && $coupon->fields['coupon_type']=='S') ) {
        if ($coupon->fields['coupon_minimum_order'] <= $orderTotal['totalFull']) {
          ...
          switch ($this->calculate_tax) {
            ...
            $ratioTax = $adjustedTax ? $adjustedTax/$order->info['tax'] : 0;
            reset($order->info['tax_groups']);
            $tax_deduct = 0;
            foreach ($order->info['tax_groups'] as $key=>$value) {
