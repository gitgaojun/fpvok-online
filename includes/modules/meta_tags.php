...
switch ($_GET['main_page']) {
  ...
  define('META_TAG_TITLE', 'Wholesale '.$_GET['keyword'].'- China Wholesale '.$_GET['keyword'].' Electronics from China');
  define('META_TAG_DESCRIPTION', 'Wholesale '.$_GET['keyword'].' from China, Dropship wholesale '.$_GET['keyword'].', wholesale '.$_GET['keyword'].' Cheapest');
  define('META_TAG_KEYWORDS', 'China wholesale,wholesale electronics,Chinese wholesaler, dropship Search Results');
   break;
  case 'account_edit':
  ...
  if (!$category_metatags->EOF) {
    ...
    if ($category_depth == 'nested') {
      ...
      if ($category_metatags->EOF) {
        ...
        define('META_TAG_TITLE', SEO_COMMON_KEYWORDS.' '.str_replace('"','', zen_clean_html($category_metatags->fields['categories_name']) . PRIMARY_SECTION . SEO_COMMON_KEYWORDS_ASSISTANCE2 . ' ' . SEO_COMMON_KEYWORDS . ' ' . zen_clean_html($category_metatags->fields['categories_name']). PRIMARY_SECTION . SEO_COMMON_KEYWORDS . ' ' . SEO_COMMON_KEYWORDS_ASSISTANCE . ' ' . zen_clean_html($category_metatags->fields['categories_name']) . FOURTH_SECTION . SEO_COMMON_KEYWORDS_ASSISTANCE2 . PRIMARY_SECTION . TITLE . TAGLINE));
  ...
  //  case 'product_info':
  case (strstr($_GET['main_page'], 'product_') or strstr($_GET['main_page'], 'document_') or strstr($_GET['main_page'], 'print_')):
  /*
  ...
  */
  ...
  $sql= "select pd.products_name, p.products_model, p.products_price_sorter, p.products_tax_class_id,
                                      ...
                                      mtpd.metatags_title, mtpd.metatags_keywords,mtpd.metatags_bottom_keywords, mtpd.metatags_description
  ...
  if ($product_info_metatags->EOF) {
    ...
    if (!empty($product_info_metatags->fields['metatags_keywords']) or !empty($product_info_metatags->fields['metatags_description'])) {
      ...
      define('META_TAG_TITLE', str_replace('"','',SEO_COMMON_KEYWORDS_ASSISTANCE . ' ' . SEO_COMMON_KEYWORDS_ASSISTANCE2 . ' ' . SEO_COMMON_KEYWORDS . ' ' .$review_on . $meta_products_name . $meta_products_price . PRIMARY_SECTION . TITLE . TAGLINE));
