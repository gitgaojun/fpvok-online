          ...
          where    patrib.products_id='" . (int)$_GET['products_id'] . "'
            ...
            if ($pr_attr->fields['total'] > 0) {
              ...
              while (!$products_options_names->EOF) {
                ...
                $options_typezx[] = array('id' => $products_options_names->fields['products_options_id'],
                  'text' => $products_options_names->fields['products_options_type']);
                /*
                ...
                while (!$products_options->EOF) {
                  ...
                  if (((CUSTOMERS_APPROVAL == '2' and $_SESSION['customer_id'] == '') or (STORE_STATUS == '1')) or ((CUSTOMERS_APPROVAL_AUTHORIZATION == '1' or CUSTOMERS_APPROVAL_AUTHORIZATION == '2') and $_SESSION['customers_authorization'] == '') or (CUSTOMERS_APPROVAL == '2' and $_SESSION['customers_authorization'] == '2') or (CUSTOMERS_APPROVAL_AUTHORIZATION == '2' and $_SESSION['customers_authorization'] != 0) ) {
                    ...
                    if ($products_options->fields['options_values_price'] != '0' and ($products_options->fields['product_attribute_is_free'] != '1' and $product_info->fields['product_is_free'] != '1')) {
					  ...
					  if($products_options_names->fields['products_options_type'] == PRODUCTS_OPTIONS_TYPE_CHECKBOX or $products_options_names->fields['products_options_type'] == PRODUCTS_OPTIONS_TYPE_RADIO){
                      $products_options_display_price= '<strong class="red">'.ATTRIBUTES_PRICE_DELIMITER_PREFIX . $products_options->fields['price_prefix'] .
                      $currencies->display_price($new_attributes_price, zen_get_tax_rate($product_info->fields['products_tax_class_id'])) . ATTRIBUTES_PRICE_DELIMITER_SUFFIX.'</strong>';
					  }else{
...
//                        $selected_attribute = ($products_options->fields['attributes_default']=='1' ? true : false);
                ...
                switch (true) {
                  ...
                  if ($show_attributes_qty_prices_icon == 'true') {
                    ...
                    $options_name[] = "<div style=\"width:350px\"><strong >". $products_options_names->fields['products_options_name']."</strong></div>";
                  }
				  ...
				  $str_id = str_replace(":","",strtolower($products_options_names->fields['products_options_name']));
				  $str_menu = "";
                  if ($show_attributes_qty_prices_icon == 'true') {
                    $options_name[] = ATTRIBUTES_QTY_PRICE_SYMBOL ."<div style=\"width:350px\"><strong style=\"width:300px\">". $products_options_names->fields['products_options_name']."</strong><div>";
                  } else {
                    $options_name[] = '<label class="attribsSelect" for="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '">' . $products_options_names->fields['products_options_name'] . '</label><input type="hidden" name="id['.$products_options_names->fields['products_options_id'].']" value="" id="'.$str_id.'_id">';
                  }
                  
				  $str_menu = '<ul id='.$str_id.'>';
				  foreach($products_options_array as $product_va){
					$str_menu .= '<li class="" ><a class="d-selected" rel="nofollow" size_id="'.$product_va['id'].'" href="javascript:vold();" data-value="S">'.$product_va['text'].'</a></li>';  
				  }
				  $str_menu .= '</ul>';
                 // $options_menu[] ='<ul id='.$str_id.'>';
                  $options_menu[] = $str_menu . "\n";
