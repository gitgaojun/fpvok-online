...
if (isset($_GET['products_id']) && SHOW_PRODUCT_INFO_COLUMNS_ALSO_PURCHASED_PRODUCTS > 0 && MIN_DISPLAY_ALSO_PURCHASED > 0) {
  ...
  if ($num_products_ordered >= MIN_DISPLAY_ALSO_PURCHASED && $num_products_ordered > 0) {
    while (!$also_purchased_products->EOF) {
      ...
      'text' => (($also_purchased_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<li><a href="' . zen_href_link(zen_get_info_page($also_purchased_products->fields['products_id']), 'products_id=' . $also_purchased_products->fields['products_id']) . '">'. zen_image_OLD(DIR_WS_IMAGES . $also_purchased_products->fields['products_image'], SEO_COMMON_KEYWORDS . ' ' .$also_purchased_products->fields['products_name'], '42', '42',' class="fl"') . '</a>') . '<span><a href="' . zen_href_link(zen_get_info_page($also_purchased_products->fields['products_id']), 'products_id=' . $also_purchased_products->fields['products_id']) . '">' . substr($also_purchased_products->fields['products_name'],0,16).'...' . '</a><br/><strong class="red">'.$currencies->display_price(zen_get_products_base_price($also_purchased_products->fields['products_id']),zen_get_tax_rate($products_tax_class_id)).'</strong></span></li>');
      $col ++;
