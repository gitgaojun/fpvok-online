...
WHERE c1.categories_id=c.categories_id and parent_id = '.$shopping_categories->fields['categories_id'] .' order by sort_order asc limit 5';
		...
		$subCatoryString .= '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '" class="more_product">More wholesale products ></a>';
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<a href="'. zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '"  class="ih fl" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">' . str_replace("/s/","/l/",zen_image(DIR_WS_IMAGES . $shopping_categories->fields['categories_image'], SEO_COMMON_KEYWORDS . ' ' .$shopping_categories->fields['categories_name'], 86, '',' class="fl"')) . '</a><div class="block pad_l_90px"><h3><a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new2) . '" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">'. $shopping_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString.'</div>');
