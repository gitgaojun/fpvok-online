...
$product_sort = " order by p.products_sort_order,p.products_recommend DESC,p.products_id desc";
...
$listTypes = isset($_GET['listtypes']) ? $_GET['listtypes'] : 2;
...
if(trim($_GET['keyword'])=='Enter search keywords here'){
...
" p.products_recommend,m.manufacturers_id, p.products_id, pd.products_name, pd.products_small_description, p.products_price,p.products_price_retail, p.product_is_wholesale, p.product_is_always_free_shipping,p.products_quantity_order_min, p.products_tax_class_id, p.products_price_sorter, p.products_qty_box_status, p.master_categories_id ";
...
$_GET['categories_id'] = intval($_GET['categories_id'])>0?intval($_GET['categories_id']):'';
...
if (isset($keywords) && zen_not_null($keywords)) {
	...
	function createKeywordsCond($cond='and',$db,$where_str,$where_str2)

	{

		$whereArr = array();	

		if (zen_parse_search_string(stripslashes($_GET['keyword']), $search_keywords,$cond))
	...
	$where_str2_orig = $where_str2;

	

	$andCond = createKeywordsCond('and',$db,$where_str_orig,$where_str2_orig);

	$where_str = $andCond[0];

	$where_str2 = $andCond[1];

	$orCond = createKeywordsCond('or',$db,$where_str_orig,$where_str2_orig);

	$where_str_or = $orCond[0];
...
$listing_sql_or = $select_str . $from_str . $where_str_or . $product_sort;
...

{
...

	{
...

		{
			...
			$kwString.=$kw.'<span class="black">+</span>';

		}

	}

	$kwString = substr($kwString,0,strlen($kwString)-strlen('<span class="black">+</span>'));
...
$kwString = intval($_GET['cagegories_id'])>0 ? zen_get_category_name(intval($_GET['cagegories_id']),$_SESSION['languages_id']):"all";

}

$breadcrumb->add('<span class="black">'.NAVBAR_TITLE_2." for </span> ".$kwString.'<span class="black"> in All Department</span>');

if($is_all==true){

$cag_id = intval($_GET['cagegories_id']);
		...
		zen_get_subcategories($subcategories_array, $cag_id);

		$where_strs = " AND p2c.products_id = p.products_id

                    AND p2c.products_id = pd.products_id

                    AND (p2c.categories_id = :categoriesID";

		$where_strs = $db->bindVars($where_strs, ':categoriesID', $cag_id, 'integer');

		for ($i=0, $n=sizeof($subcategories_array); $i<$n; $i++ ) {

			$where_strs .= " OR p2c.categories_id = :categoriesID";

			$where_strs = $db->bindVars($where_strs, ':categoriesID', $subcategories_array[$i], 'integer');
		...
		$where_strs = ' AND p2c.products_id = p.products_id AND p2c.products_id = pd.products_id ';	

		}

		

$listing_sql  = "SELECT p.products_id, p.products_type, pd.products_name, pd.products_small_description, p.products_image, p.products_price,
...
if ($advanced_search_split->number_of_rows == 0) {
	...
	if($is_all==true){
		...
		$advanced_search_split = new splitPageResults($listing_sql_or, (isset($_GET['pagesize']) ? $_GET['pagesize'] : MAX_DISPLAY_PRODUCTS_LISTING ), 'p.products_id', 'page');//change the operator to 'or'
...
if($advanced_search->RecordCount()>0){
	...
	while (!$advanced_search->EOF) {
		...
		$list_box_contents[$row]['products_model'] = $advanced_search->fields['products_model'];

		$list_box_contents[$row]['products_small_description'] = $advanced_search->fields['products_small_description'];
		...
		$list_box_contents[$row]['products_maxprice']=zen_get_products_discount_price_qty($advanced_search->fields['products_id'], 100);
		...
		$list_box_contents[$row]['products_recommend']=$advanced_search->fields['products_recommend'];
