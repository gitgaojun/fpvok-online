  ...
  if (!isset($_SESSION['customer_id'])) {
    ...
    if (zen_get_customer_validate_session($_SESSION['customer_id']) == false) {
	  ...
	  $sql = "SELECT * FROM " . TABLE_ADDRESS_BOOK . "
			  WHERE  address_book_id =(SELECT customers_default_address_id FROM " . TABLE_CUSTOMERS . "
			  WHERE  customers_id = :customersID)";
	  $sql = $db->bindVars($sql, ':customersID', $_SESSION['customer_id'], 'integer');
	  $isphone=$db->Execute($sql);
	  $book_id=$isphone->fields['address_book_id'];
	if($_GET['checkout']==1){
		  if(strlen($isphone->fields['entry_phone'])>=2){
		   zen_redirect(zen_href_link('checkout_shipping', '', 'SSL'));
		  }else{
		   zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, 'has_phone=no_phone', 'SSL'));
		  }
	}
...
if (isset($_POST['action']) && (($_POST['action'] == 'createShipping') || ($_POST['action'] == 'update'))) {
	...
	$state = zen_db_prepare_input($_POST['state']);
	$has_phone = zen_db_prepare_input($_POST['has_phone']);
	$isSame = zen_db_prepare_input($_POST['isSame']);
    if($has_phone=='no_phone' && $isSame!='true'){
    $b_gender = zen_db_prepare_input($_POST['b_gender']);
    $b_firstname = zen_db_prepare_input($_POST['b_firstname']);
    $b_lastname = zen_db_prepare_input($_POST['b_lastname']);
    $b_street_address = zen_db_prepare_input($_POST['b_street_address']);
    $b_suburb = zen_db_prepare_input($_POST['b_suburb']);
    $b_postcode = zen_db_prepare_input($_POST['b_postcode']);
    $b_phone = zen_db_prepare_input($_POST['b_phone']);
    $b_city = zen_db_prepare_input($_POST['b_city']);
    $b_state = zen_db_prepare_input($_POST['b_state']);
	}
    ...
    if (isset($_POST['zone_id'])) {
	  ...
	  if($isSame!='true')$b_zone_id = zen_db_prepare_input($_POST['b_zone_id']);
    } else {
      $zone_id = false;
	  if($isSame!='true')$b_zone_id = false;
    }
    //if not
    $country = zen_db_prepare_input($_POST['zone_country_id']);
	if($isSame!='true')$b_country = zen_db_prepare_input($_POST['b_zone_country_id']);
    /**
    ...
    if ($error == false) {
		...
		if($has_phone=='no_phone' && $isSame!='true'){
			...
			$sql_data_array2= array(array('fieldName'=>'entry_gender', 'value'=>$b_gender, 'type'=>'enum:m|f'),
								   array('fieldName'=>'entry_firstname', 'value'=>$b_firstname, 'type'=>'string'),
								   array('fieldName'=>'entry_lastname', 'value'=>$b_lastname, 'type'=>'string'),
								   array('fieldName'=>'entry_street_address', 'value'=>$b_street_address, 'type'=>'string'),
								   array('fieldName'=>'entry_suburb', 'value'=>$b_suburb, 'type'=>'string'),
								   array('fieldName'=>'entry_postcode', 'value'=>$b_postcode, 'type'=>'string'),
								   array('fieldName'=>'entry_phone', 'value'=>$b_phone, 'type'=>'string'),
								   array('fieldName'=>'entry_city', 'value'=>$b_city, 'type'=>'string'),
								   array('fieldName'=>'entry_country_id', 'value'=>$b_country, 'type'=>'integer'));
	  
			if ($zone_id > 0) {
			  $sql_data_array2[] = array('fieldName'=>'entry_zone_id', 'value'=>$b_zone_id, 'type'=>'integer');
			  $sql_data_array2[] = array('fieldName'=>'entry_state', 'value'=>'', 'type'=>'string');
			} else {
			  $sql_data_array2[] = array('fieldName'=>'entry_zone_id', 'value'=>'0', 'type'=>'integer');
			  $sql_data_array2[] = array('fieldName'=>'entry_state', 'value'=>$b_state, 'type'=>'string');
			}
			
		}
		if($has_phone=='no_phone'){
				if($isSame=='true'){
				$sql_data_array3 = array(array('fieldName'=>'customers_gender', 'value'=>$gender, 'type'=>'string'),
										...
										array('fieldName'=>'customers_telephone', 'value'=>$phone, 'type'=>'string'));
				}else{
				$sql_data_array3 = array(array('fieldName'=>'customers_gender', 'value'=>$b_gender, 'type'=>'string'),
										array('fieldName'=>'customers_firstname', 'value'=>$b_firstname, 'type'=>'string'),
										array('fieldName'=>'customers_lastname', 'value'=>$b_lastname, 'type'=>'string'),
										array('fieldName'=>'customers_telephone', 'value'=>$b_phone, 'type'=>'string'));
				}
		}
    ...
    if ($_POST['action'] == 'update') {
      ...
      if($has_phone=='no_phone'){
		  ...
		  $where_clause = "address_book_id = :edit and customers_id = :customersID";
		  $where_clause = $db->bindVars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
		  $where_clause = $db->bindVars($where_clause, ':edit', $book_id, 'integer');
		  $db->perform(TABLE_ADDRESS_BOOK, $sql_data_array, 'update', $where_clause);
		$where_clause = "customers_id = :customersID";
		$where_clause = $db->bindVars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
		  $db->perform(TABLE_CUSTOMERS, $sql_data_array3, 'update', $where_clause);
		  $_SESSION['customer_first_name'] = $firstname;
		  }else{
		  $where_clause = "address_book_id = :edit and customers_id = :customersID";
		  $where_clause = $db->bindVars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
		  $where_clause = $db->bindVars($where_clause, ':edit', $book_id, 'integer');
		  $db->perform(TABLE_ADDRESS_BOOK, $sql_data_array2, 'update', $where_clause);
		$where_clause = "customers_id = :customersID";
		$where_clause = $db->bindVars($where_clause, ':customersID', $_SESSION['customer_id'], 'integer');
		  $db->perform(TABLE_CUSTOMERS, $sql_data_array3, 'update', $where_clause);
		  $_SESSION['customer_first_name'] = $b_firstname;
		  }
	  }
