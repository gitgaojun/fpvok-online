...
//  $display_limit = zen_get_free_shipping_timelimit();
  ...
  $free_shipping_query_raw = "SELECT p.products_id, p.products_type, pd.products_name,pd.products_small_description, p.products_image, p.products_price,
                             ...
                             AND pd.language_id = :languageID " .$displayOrder. $display_limit . $pricefilter . $product_sort ;

  $free_shipping_query_raw = $db->bindVars($free_shipping_query_raw, ':languageID', $_SESSION['languages_id'], 'integer');
 // echo $free_shipping_query_raw;
...
//check to see if we are in normal mode ... not showcase, not maintenance, etc
  ...
  if($free_shipping->RecordCount()>0){
    ...
    while (!$free_shipping->EOF) {
      ...
      $list_box_contents[$row]['products_small_description'] = $free_shipping->fields['products_small_description'];      
      ...
      $list_box_contents[$row]['products_recommend']=$free_shipping->fields['products_recommend'];
...
//check to see if we are in normal mode ... not showcase, not maintenance, etc
  ...
  if($free_shipping2->RecordCount()>0){
    ...
    while (!$free_shipping2->EOF) {
      ...
      $list_box_contents2[$row]['products_description'] = zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($free_shipping2->fields['products_id'], $_SESSION['languages_id']))), 230);
      ...
      $list_box_contents2[$row]['products_recommend']=$free_shipping2->fields['products_recommend'];
