...
*/
      if(RSS_PRODUCTS_IMAGES == 'true' && zen_not_null($products->fields['products_image'])) {
		...
		$products_image=explode(",",$products_image);
		$products_image_medium = explode(",",$products_image_medium);
		$products_image_large = explode(",",$products_image_large);
		
		$products_image=$products_image[0];
		$products_image_medium = $products_image_medium[0];
		$products_image_large = $products_image_large[0];
        switch ($imageSize) {
          case 'small':
