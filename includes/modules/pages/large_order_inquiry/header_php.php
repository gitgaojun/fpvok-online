...
if (isset($_GET['action']) && ($_GET['action'] == 'send')) {
	    echo "<script>alert('hello,here');</script>";
  ...
  if ($zc_validate_email and !empty($enquiry) and !empty($name)) {
    ...
    if($_SESSION['customer_id']) {
      $sql = "SELECT customers_id, customers_firstname, customers_lastname,customers_telephone, customers_password, customers_email_address, customers_default_address_id 
    ...
    "\n\n" .  
    'Target Price:'."\t".$_POST['target_price']. "\n\n" .
    ORDER_QUANTITY."\t".$_POST['order_quantity']. "\n\n" .
    ENTRY_PHONE_NUMBER."\t".$_POST['phone']. "\n\n" .
    ...
    zen_redirect(zen_href_link(FILENAME_LARGE_ORDER_INQUIRY, 'action=success&products_id='.$_GET['products_id']));
  } else {
    $error = true;
    if ($error) {
...
$define_page = zen_get_file_directory(DIR_WS_LANGUAGES . $_SESSION['language'] . '/html_includes/', 'define_large_order_inquiry', 'false');
