 ...
 */
  ...
  $module_show_categories = PRODUCT_INFO_CATEGORIES;
	...
	if($_GET['pid'] !="") {
	 $str_count = zen_get_diggid_unit($_GET['pid']) +1;
	 $sql = "update " . TABLE_PRODUCTS . " set products_diggs = '" . $str_count . "'
  ...
  if ( $res->fields['total'] < 1 ) {
    ...
    $sql = "select p.products_id, pd.products_name,p.accessories_product,
                  pd.products_description, pd.products_description1, pd.products_description2, p.products_model,
                  p.products_quantity,p.products_image,
                  pd.products_url, p.products_price,p.products_diggs,
...
// if review must be approved or disabled do not show review
    $review_status = " and r.status = '1' order by r.date_added desc";

    $reviews_query = "select r.reviews_id,r.reviews_pic,r.customers_name,r.reviews_rating,r.date_added,r.reviews_is_featured,rd.reviews_title,rd.reviews_text,rd.reviews_admin from " . TABLE_REVIEWS . " r, "
    ...
    $ask_status = " and a.status = '1' order by a.date_added desc";
    $asks_query = "select a.reviews_id,a.reviews_pic,a.customers_name,a.date_added,a.reviews_is_featured,ad.reviews_title,ad.reviews_text,ad.reviews_admin from " . TABLE_ASK . " a, "  . TABLE_ASK_DESCRIPTION . " ad
                       where a.products_id = '" . (int)$_GET['products_id'] . "'
                       and a.reviews_id = ad.reviews_id
                       and ad.languages_id = '" . (int)$_SESSION['languages_id'] . "'" .
                       $ask_status;
    
    $asks = $db->Execute($asks_query);
	$ask_countttt = $asks->RecordCount();
  ...
  $products_description1 = $product_info->fields['products_description1'];
  ...
  $products_diggs = $product_info->fields['products_diggs'];
  //$products_sold = $product_info->fields['products_sold'];
  $accessories_product = $product_info->fields['accessories_product'];
