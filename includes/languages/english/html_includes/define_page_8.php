<div class="topseller mt10 w0">
        <div class="topseller_h1 w0">Weekly Bestseller Clearance Items</div>
		...
		$product_in_categories_sql = '( p.master_categories_id ='.$product_in_categories_sql.')';			
		    ...
		    $featured_products_query = "select p.products_id, p.products_image, pd.products_name,p.products_price
			...
			if ($specials_index->RecordCount() > 0) {
				while (!$specials_index->EOF) {
					...
					$products_price = $specials_index->fields['products_price'];			   
						   ?>
            <li class="w202"><span class="topseller_pic fleft"><a href="<?php echo $url ?>">
               <?php echo zen_image(DIR_WS_IMAGES . $specials_index->fields['products_image'], SEO_COMMON_KEYWORDS . ' ' .zen_get_products_name($specials_index->fields['products_id']), 45, ' ',' class="fl"'); ?></a></span> <span class="topseller_pro_name w136 fleft">
                    <a href="<?php echo $url; ?>" class="f12"><?php echo $specials_index->fields['products_name']; ?></a></span>
                <span currencycode="USD" class="topseller_price fleft cf50 f11"><?php echo $symbolLeft.'&nbsp;'.$currencies->noSymbolDisplayPrice($products_price,0); ?></span>
                <div class="clear">
                </div>
            </li>
