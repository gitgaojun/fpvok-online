      ...
      while (list(, $value) = each($this->modules)) {
        ...
        if ($return_html == true) {
          for ($i=0; $i<$size; $i++) {
            ...
            $output_string .= '<tr>'."\n".
                              '<td align="right" style="padding-left:70px;">' . $GLOBALS[$class]->output[$i]['title'] . ' </td>' . "\n" .
                              '<td id="'.str_replace('_', '-', $GLOBALS[$class]->code).'" align="left">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                              '</tr>';
            }else{
			$output_string .= '<tr style="display:none;" id="Shipping_insurances">
				  <td align="right">Shipping Insurance:</td>
				  <td >'. $_SESSION['totalcol'] .'</td>
				</tr>';
			$output_string .= '<tr style="display:none;" id="tracking_id">
				  <td align="right" style="padding-left:70px;">Tracking numver:</td>
				  <td id="ot-chinafee" align="left">'.$_SESSION['totalcol_fee'].'</td>
				</tr>';
            $output_string .= '<tr>'."\n".
                              ...
                              '<h3 style="margin-left: 207px;" class="g_t_c">' . $GLOBALS[$class]->output[$i]['title'].'&nbsp;<span id="'.str_replace('_', '-', $GLOBALS[$class]->code).'">' . $GLOBALS[$class]->output[$i]['text'] . '</span></h3></td>' . "\n" .
                              '</tr>';          	
            }
          }
