...
class shoppingCart extends base {
  ...
  function restore_contents() {
    ...
    while (!$products->EOF) {
		  ...
		  $check_p = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . $products->fields['products_id'] . "'";
	
		  $check1 = $db->Execute($check_p);	
		  
   //  是否有搭配，0 正常, 1主产品有搭配，2搭配件
   		  $is_with =0;
		  
   
		  if(strstr($check1->fields['accessories_product'],"#")){
			  

				$products_query2 = "select products_id, customers_basket_quantity
									 from " . TABLE_CUSTOMERS_BASKET . "
									 where customers_id = '" . (int)$_SESSION['customer_id'] . "'";
			
				$products2 = $db->Execute($products_query2);
			
				while (!$products2->EOF) {
					
					if(strstr($check1->fields['accessories_product'].'#',$products2->fields['products_id'].'#')){
						$is_with = 1;
			...
			//是否配件判断  
				...
				$products_query3 = "select products_id, customers_basket_quantity
									 from " . TABLE_CUSTOMERS_BASKET . "
									 where customers_id = '" . (int)$_SESSION['customer_id'] . "'";
			
				$products3 = $db->Execute($products_query3);
			
				while (!$products3->EOF) {
				  $check_p2 = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . $products3->fields['products_id'] . "'";
			
				  $check2 = $db->Execute($check_p2);	
		  
					if(strstr($check2->fields['accessories_product'].'#',$products->fields['products_id'].'#')){
						$is_with = 2;
	  ...
	  $this->contents[$products->fields['products_id']]['w_id'] = $is_with;
      // attributes
   ...
   */
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true,$with_str=0) {
    global $db;
    ...
    if ($this->in_cart($products_id)) {
	  ...
	  $this->update_quantity($products_id, $qty, $attributes,$with_str);
    } else {
		
      $this->contents[] = array($products_id);
	  
      $this->contents[$products_id]['qty'] =(float)$qty;
	  $this->contents[$products_id]['w_id'] = $with_str;  
   ...
   * @global object access to the db object
   */
  function update_quantity($products_id, $quantity = '', $attributes = '',$with_str='') {
    global $db;
  ...
  function remove($products_id) {
    ...
    //      $products_id = zen_get_uprid($products_id, $attributes);
	if($this->contents[$products_id]['w_id']!=2){
		unset($this->contents[$products_id]);
		if (is_array($this->contents)) {

		$check_p4 = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'";
		$check4 = $db->Execute($check_p4);
		  while (list($products_id2, ) = each($this->contents)) { 
				if(strstr($check4->fields['accessories_product'].'#',$products_id2.'#')){
					$this->contents[$products_id2]['w_id'] = 0;
				}
  ...
  function calculate() {
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      $product_query = "select products_id, products_price, products_price_sample, products_tax_class_id, products_weight,
                          products_priced_by_attribute,products_price_dapei, product_is_always_free_shipping, products_discount_type, products_discount_type_from,
      ...
      if ($product = $db->Execute($product_query)) {
		...
		$products_old_price = ($products->fields['products_price'] == 0 ? $products->fields['products_price_sample'] :$products->fields['products_price']) ;
		$products_price = $product->fields['products_price_dapei'];	
		}else{
		$products_old_price =0;	
  ...
  function get_products($check_for_valid_cart = false) {
    ...
    while (list($products_id,) = each($this->contents)) {
      $products_query = "select p.products_id, p.master_categories_id, p.products_status, pd.products_name, p.products_model, p.products_image,
                                  ...
                                  p.product_is_free, p.products_priced_by_attribute,p.products_price_dapei,
      ...
      if ($products = $db->Execute($products_query)) {
		...
		if($this->contents[$products_id]['w_id']==2){
		$products_old_price = ($products->fields['products_price'] == 0 ? $products->fields['products_price_sample'] :$products->fields['products_price']) ;
		$products_price = ($products->fields['products_price_dapei'] == 0 ? $products->fields['products_price_sample'] :$products->fields['products_price_dapei']) ;	
		}else{
		$products_old_price =0;	
        $products_price = ($products->fields['products_price'] == 0 ? $products->fields['products_price_sample'] :$products->fields['products_price']) ;
		}
...
// validate cart contents for checkout
        ...
        $products_array[] = array('id' => $products_id,
								  'w_id'=>$this->contents[$products_id]['w_id'],
...
header('Location: ' . zen_href_link($goto, zen_get_all_get_params($parameters)));
      //zen_redirect();
    } else {
  ...
  function actionBuyNow($goto, $parameters) {
    ...
    if (isset($_GET['products_id'])) {
      if (zen_has_product_attributes($_GET['products_id']) && 3>6) {
...
//    zen_redirect(zen_href_link($goto, zen_get_all_get_params($parameters)));
  ...
  function actionMultipleAddProduct($goto, $parameters) {
	...
	/*if (is_array($_POST['products_idt']) && sizeof($_POST['products_idt']) > 0) {
      foreach($_POST['products_idt'] as $key=>$val) {
//      while ( list( $key, $val ) = each($_POST['products_id']) ) {
        if ($val > 0 && isset($_POST['products_'.$key]) && $_POST['products_'.$key]==1) {
          $adjust_max = false;
          $prodId = $key;
          $qty = $val;
...
//echo 'I SEE actionMultipleAddProduct: ' . $prodId . '<br>';
          ...
          if (($add_max == 1 and $cart_qty == 1)) {
            ...
            $this->add_cart($prodId, $this->get_quantity($prodId)+($new_qty),$real_ids[$prodId]);
          }
          if ($adjust_max == 'true') {
...
/*	
	 if (is_array($_POST['order_id']) && sizeof($_POST['order_id']) > 0) {
		 $str="";
		 foreach($_POST['order_id'] as $names){
			$str.= $names.',';
		 }
		 $str=trim($str,",");
		 $_SESSION['del_id'] = $str;
		 $A_productid=array();
		 $statuses_query = "SELECT products_id,products_quantity ,orders_id	
					   FROM   " . TABLE_ORDERS_PRODUCTS . "
					   WHERE orders_id in(".$str.")
					   ORDER BY orders_id asc";
		$statuses = $db->Execute($statuses_query);
		while (!$statuses->EOF) {
			
			$sql = "update orders set orders_status=101
                  where orders_id='" . (int)$statuses->fields['orders_id'] . "'";
            $db->Execute($sql);
			
		$A_productid[$statuses->fields['products_id']]+=$statuses->fields['products_quantity'];
		$statuses->MoveNext();
		}
		$_POST['products_id']=$A_productid;  
	 }	
	*/
...
//echo 'I SEE actionMultipleAddProduct: ' . $prodId . '<br>';
		  ...
		  $check_p = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . $prodId . "'";
   ...
   //  是否有搭配，0 正常, 1主产品有搭配，2搭配件
		  ...
		  if(strstr($check1->fields['accessories_product'],"#")){
			 ...
			 //一起加入的产品判断
			  foreach($_POST['products_id'] as $key2=>$val2) {
				  if(strstr($check1->fields['accessories_product'].'#',$key2.'#')){
					  $is_with = 1;
			...
			//已加购物车判断		  
				if (is_array($this->contents)) {
				  reset($this->contents);
				  while (list($products_id4, ) = each($this->contents)) { 
						$products_ids = intval($products_id4);
						
						if(strstr($check1->fields['accessories_product'].'#',$products_ids.'#')){
							$is_with = 1;
							$this->contents[$products_id4]['w_id'] = 2;
			...
			//是否配件判断  
			 ...
			 //一起加入的产品判断
			  foreach($_POST['products_id'] as $key2=>$val2) {
				  ...
				  $check_p2 = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . $key2 . "'";
			
				  $check2 = $db->Execute($check_p2);	
				  
				  if(strstr($check2->fields['accessories_product'].'#',$prodId.'#')){
					  $is_with = 2;  
			...
			//已加购物车判断		  
				if (is_array($this->contents)) {
				  reset($this->contents);
				  while (list($products_id3, ) = each($this->contents)) { 
						$products_ids2 = intval($products_id3);
						
						$check_p3 = "select accessories_product from " . TABLE_PRODUCTS . " where products_id = '" . $products_ids2 . "'";
					
						$check3 = $db->Execute($check_p3);	
				  
						if(strstr($check3->fields['accessories_product'].'#',$prodId.'#')){
							$is_with = 2;
							$this->contents[$products_ids2]['w_id'] =1;
		  ...
		  if($_POST['products_id_d']==$prodId){
			  
			  $real_ids = isset($_POST['id']) ? $_POST['id'] : "";
		  }else{
			  if(isset($_POST['id'])){
				  unset($real_ids);
				 foreach($_POST['id'] as $key5=>$val5){
					// echo $key5.'#'.$prodId;
					$aac = strval($key5);
					$aab = strval($prodId);

					if(str_replace($aab,'',$aac)!=$aac){
						$key5 = str_replace($prodId,"",$key5);
						$real_ids[$key5] = $val5;
					}
				 }
			...
			switch($is_with){
				...
				break;
				case 1:
					$this->add_cart($prodId, $this->get_quantity(zen_get_uprid($prodId, $real_ids))+($new_qty),$real_ids,true,1);
				break;
				case 2:
					$this->add_cart($prodId, $this->get_quantity(zen_get_uprid($prodId, $real_ids))+($new_qty),$real_ids,true,2);
				break;
			}
