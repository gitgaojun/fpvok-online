 ...
 */
  $content = '<div class="gray_box border_bottom_ddd">';
  $content .= '<table>' . "\n";
  for ($i=1; $i<=sizeof($bestsellers_list); $i++) {
		$content .= '<tr>';
    $content .= '<td width="45"><a href="' . zen_href_link(zen_get_info_page($bestsellers_list[$i]['id']), 'products_id=' . $bestsellers_list[$i]['id']) . '">'.zen_image(DIR_WS_IMAGES . $bestsellers_list[$i]['products_image'], SEO_COMMON_KEYWORDS . ' ' .$bestsellers_list[$i]['name'], 42, 42 ,'align="absmiddle"') .'</a></td><td><span class="line_120"><a href="' . zen_href_link(zen_get_info_page($bestsellers_list[$i]['id']), 'products_id=' . $bestsellers_list[$i]['id']) .'">'. zen_trunc_string($bestsellers_list[$i]['name'], BEST_SELLERS_TRUNCATE, BEST_SELLERS_TRUNCATE_MORE) . '</a><strong class="red"><br/>'.$bestsellers_list[$i]['products_price'].'</strong></span></td></tr>' . "\n";
  }
  $content .= '</table></div>';
?>