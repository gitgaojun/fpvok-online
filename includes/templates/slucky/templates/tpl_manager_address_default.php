...
<div class="BoxBar">My Account</div>
	...
	<li><a class="red b"href="<?php echo zen_href_link(FILENAME_MANAGER_ADDRESS,'','SSL');?>">My Address</a></li>
  ...
  <tr>
    ...
    <td colspan="3">
      ...
      <tr id="zone_id_tr1" style="dispaly:block" >
       <td align="right"><strong><?php echo ENTRY_STATE; ?></strong></td>
       <td colspan="3">
	   <?php if($entry->fields['entry_zone_id']>0){?>
         <input type="text" id="state" class="l_input" maxlength="40" value="<?php echo $entry->fields['entry_zone_id']; ?>" name="state"/>
		 <?php }else{?>
		 <input type="text" id="state" class="l_input" maxlength="40" value="<?php echo $entry->fields['entry_state']; ?>" name="state"/>
		 <?php }?>
         </td>
    </tr>
        <tr>
...
<?php }else{ ?>
<div class="pad_l_30px margin_t big allborder">
...
<div class="margin_t">
<?php echo '<a href="'.zen_href_link(FILENAME_MANAGER_ADDRESS,'edit='.$_SESSION['customer_default_address_id'].'','SSL').'">'.zen_image($template->get_template_dir('btn_edit.gif', DIR_WS_TEMPLATE, $current_page_base,'images/button'). '/btn_edit.gif').'</a>'; ?>
</div>
</ul>
