...
var google_conversion_label = "YgagCOD38gEQ2Izr3AM";
...
<img height="1" width="1" style="border-style:none;" alt="" src="http://www.googleadservices.com/pagead/conversion/999999064/?label=YgagCOD38gEQ2Izr3AM&amp;guid=ON&amp;script=0"/>
</div>
</noscript>
<ul id="projects">
...
<h3 class="check_box_tit black pad_1em">Address Book</h3>
...
<?php
  if ($process == false || $error == true) {
	  ...
	  echo '<ul class="clear">';
	  echo '<div>';
	  echo '<h4>Shipping Address:</h4><SPAN class=red>*</SPAN> Indicates required fields';
	  echo '</div>';
	  ...
	  echo zen_draw_form('checkout_address', zen_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL'), 'post', 'onsubmit="return(fmChk(this))" id="checkout_address"');
		...
		echo zen_draw_hidden_field('has_phone','no_phone');
	  if($addresses_count < MAX_ADDRESS_BOOK_ENTRIES) {
		  require($template->get_template_dir('tpl_modules_checkout_new_address2.php', DIR_WS_TEMPLATE, $current_page_base,'templates'). '/' . 'tpl_modules_checkout_new_address2.php');
		}
  echo '</form>';
...
<br class="clear"/>
  ...
  <SCRIPT>
tosame();
update_billing_zone($('checkout_address'));
initForm('checkout_address','tosame()');
</SCRIPT>  	
<?php } else{?>
<div class="margin_t allborder">
