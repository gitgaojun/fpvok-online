...
<?php echo zen_draw_form('estimator', 'index.php?main_page=shipping_estimator', 'post','target="_self"'); ?>
...
?>
<?php if (strlen($order->delivery['street_address'])>0){; ?>
...
?>
<table width="100%"  cellpadding="0" cellspacing ="0" id="table_info_l" class="in_1em margin_t">
     <tr>
       <th scope="col" width="300" align="center"><?php echo CART_SHIPPING_METHOD_TEXT; ?></th>
       <th scope="col" width="140" align="center">Shipping Fee</th>
       <th scope="col"  align="center"><?php echo CART_SHIPPING_METHOD_TOTAL_COST; ?></th>
     </tr>
<?php
...
?>
         ...
         <td><?php echo $currencies->format((zen_add_tax($_SESSION['cart']->show_total(), $quotes[$i]['tax']))).'&nbsp;+&nbsp;'.$currencies->format(zen_add_tax($quotes[$i]['methods'][0]['cost'], $quotes[$i]['tax'])).'&nbsp;=&nbsp;'.$currencies->format($_SESSION['cart']->show_total()+zen_add_tax($quotes[$i]['methods'][0]['cost'], $quotes[$i]['tax'])); ?></td>
       </tr>
<?php
            }else{
?>
...
<?php
              ...
              if($selected_shipping['id'] == $thisquoteid){
?>
         <td><strong><?php //echo $quotes[$i]['module']; ?>&nbsp;<?php echo $quotes[$i]['methods'][$j]['title']; ?></strong></td>
         <td><strong><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])); ?></strong></td>
		 <td><strong><?php echo $currencies->format((zen_add_tax($_SESSION['cart']->show_total(), $quotes[$i]['tax']))).'&nbsp;+&nbsp;'.$currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])).'&nbsp;=&nbsp;'.$currencies->format($_SESSION['cart']->show_total()+zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])); ?></strong></td>
       </tr>
<?php
              }else{
?>
        <td><?php //echo $quotes[$i]['module']; ?>&nbsp;<?php echo $quotes[$i]['methods'][$j]['title']; ?>
        </td>
        <td><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])); ?></td>
		<td><?php echo $currencies->format((zen_add_tax($_SESSION['cart']->show_total(), $quotes[$i]['tax']))).'&nbsp;+&nbsp;'.$currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])).'&nbsp;=&nbsp;'.$currencies->format($_SESSION['cart']->show_total()+zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])); ?></td>
      </tr>
<?php
              }
            }
