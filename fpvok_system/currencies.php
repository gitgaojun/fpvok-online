  ...
  if (zen_not_null($action)) {
    ...
    switch ($action) {
      ...
      case 'save':
		...
		$cur_sort = zen_db_prepare_input($_POST['cur_sort']);
      ...
      case 'update':
        ...
        $currency = $db->Execute("select currencies_id, code, title,cur_sort from " . TABLE_CURRENCIES);
...
<?php
  ...
  $currency_query_raw = "select currencies_id, title,cur_sort, code, symbol_left, symbol_right, decimal_point, thousands_point, decimal_places, last_updated, value from " . TABLE_CURRENCIES . " order by title";
...
<?php
  ...
  switch ($action) {
    ...
    case 'new':
	  ...
	  $contents[] = array('text' => '<br>' . '排序' . '<br>' . zen_draw_input_field('cur_sort'));
    ...
    case 'edit':
	  ...
	  $contents[] = array('text' => '<br>' . '排序' . '<br>' . zen_draw_input_field('cur_sort', $cInfo->cur_sort));
    ...
    default:
      ...
      if (is_object($cInfo)) {
		...
		$contents[] = array('text' => '排序' . '：' . $cInfo->cur_sort);
