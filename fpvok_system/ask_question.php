 ...
 */
  ...
  if (zen_not_null($action)) {
    ...
    switch ($action) {
      ...
      case 'setflag':
        zen_set_asks_status($_GET['id'], $_GET['flag']);

        zen_redirect(zen_href_link(FILENAME_ASK, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'rID=' . $_GET['id'], 'NONSSL'));
        break;
      case 'featured':
      ...
      case 'deleteconfirm':
        ...
        if (zen_admin_demo()) {
          ...
          zen_redirect(zen_href_link(FILENAME_ASK, 'page=' . $_GET['page']));
        }
        if (isset($_POST['rID']) && is_array($_POST['rID']) && count($_POST['rID']) > 0)
			{
				foreach ($_POST['rID'] as $getRid)
				{
					$db->Execute("delete from " . TABLE_ASK . "
                      where reviews_id = '" . (int) $getRid . "'");
					
					$db->Execute("delete from " . TABLE_ASK_DESCRIPTION . "
                      where reviews_id = '" . (int) $getRid . "'");
				}
			}
			else
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="2">
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr><?php echo zen_draw_form('search', FILENAME_ASK, '', 'get'); ?>
...
// show reset search
  if (isset($_GET['search']) && zen_not_null($_GET['search'])) {
    echo '<a href="' . zen_href_link(FILENAME_ASK) . '">' . zen_image_button('button_reset.gif', IMAGE_RESET) . '</a>&nbsp;&nbsp;';
  }
...
?>
					...
					<tr><?php echo zen_draw_form('status', FILENAME_ASK, '', 'get', '', true); ?>
...
<?php
  if ($action == 'edit') {
    ...
    $reviews = $db->Execute("select r.reviews_id, r.products_id, r.customers_name,r.reviews_pic, r.date_added,
                                    r.last_modified, r.reviews_read, rd.reviews_text,rd.reviews_admin, r.reviews_rating,r.customers_email
                             from " . TABLE_ASK . " r, " . TABLE_ASK_DESCRIPTION . " rd
...
?>
      <tr><?php echo zen_draw_form('review', FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $_GET['rID'] . '&action=preview'); ?>
        <td>
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
...
?>
      <tr><?php echo zen_draw_form('update', FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $_GET['rID'] . '&action=update', 'post', 'enctype="multipart/form-data"'); ?>
        <td>
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
...
?>
      <tr>
				<td align="right" class="smallText"><?php echo '<a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . zen_image_button('button_back.gif', IMAGE_BACK) . '</a> ' . zen_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
				</form>
			</tr>
<?php
    } else {
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						...
						<form name="reviewCheck"
							action="<?php echo zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&action=delete');?>"
							method="POST">
						<table border="0" width="100%" cellspacing="0" cellpadding="2">
...
// create search filter
    ...
    $reviews_query_raw = ("select r.*, rd.*, pd.*, p.* from (" . TABLE_ASK . " r left join " . TABLE_ASK_DESCRIPTION . " rd on r.reviews_id = rd.reviews_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on r.products_id = pd.products_id and pd.language_id ='" . (int)$_SESSION['languages_id'] . "' left join " . TABLE_PRODUCTS . " p on p.products_id= r.products_id) " . " where r.products_id = p.products_id " . $search . $order_by);

//    $reviews_query_raw = "select reviews_id, products_id, date_added, last_modified, reviews_rating, status from " . TABLE_ASK . " order by date_added DESC";
    ...
    while (!$reviews->EOF) {
      if ((!isset($_GET['rID']) || (isset($_GET['rID']) && ($_GET['rID'] == $reviews->fields['reviews_id']))) && !isset($rInfo)) {
        $reviews_text = $db->Execute("select r.reviews_read, r.customers_name,r.customers_email,
      ...
      if (isset($rInfo) && is_object($rInfo) && ($reviews->fields['reviews_id'] == $rInfo->reviews_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $reviews->fields['reviews_id']) . '\'">' . "\n";
      }
      $products_name_string = zen_get_products_name($reviews->fields['products_id']);
      $products_name_string = $products_name_string; 
?>             
                <td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $reviews->fields['reviews_id'] . '&action=preview') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' .$products_name_string; ?></td>
...
<?php
      if ($reviews->fields['status'] == '1') {
        echo '<a href="' . zen_href_link(FILENAME_ASK, 'action=setflag&flag=0&id=' . $reviews->fields['reviews_id'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_green_on.gif', IMAGE_ICON_STATUS_ON) . '</a>';
      } else {
        echo '<a href="' . zen_href_link(FILENAME_ASK, 'action=setflag&flag=1&id=' . $reviews->fields['reviews_id'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_red_on.gif', IMAGE_ICON_STATUS_OFF) . '</a>';
      }
?>
                </td>							
							<td class="dataTableContent" align="right"><?php if ( (is_object($rInfo)) && ($reviews->fields['reviews_id'] == $rInfo->reviews_id) ) { echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $reviews->fields['reviews_id']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
							</tr>
<?php
      $reviews->MoveNext();
    }
...
<?php
    ...
    switch ($action) {
      case 'delete':
        ...
        {        	
          foreach ($_POST['reviewCheckBox'] as $setRId)
          {
          	...
          	$pName = $db->Execute("SELECT products_name FROM ".TABLE_ASK." r, ".TABLE_PRODUCTS_DESCRIPTION." pd WHERE r.products_id=pd.products_id AND r.reviews_id=".$setRId);
          	$products_name_texts.='<br>'.$pName->fields['products_name'];
          }
        }
        $contents = array('form' => zen_draw_form('reviews', FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=deleteconfirm').$hiddenRid);
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . zen_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      default:
      if (isset($rInfo) && is_object($rInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . zen_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a onclick="deleteReveiws(document.reviewCheck,\''.zen_href_link(FILENAME_ASK, 'page=' . $_GET['page'] . '&rID=' . $rInfo->reviews_id . '&action=delete').'\')"href="#">' . zen_image_button('button_delete.gif', IMAGE_DELETE) . '</a> ' . ' <a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . zen_get_products_category_id($rInfo->products_id) . '&pID=' . $rInfo->products_id) . '">' . zen_image_button('button_details.gif', IMAGE_DETAILS) . '</a>');
        ...
        $contents[] = array('text' => '<br>' . '评论回复:' . ' ' . $rInfo->reviews_admin);
      }
        break;
    }
