 ...
 */
  ...
  if (isset($_GET['groupfilter'])){
     ...
     $grouporder = "AND customers_describes = ".$_GET['groupfilter'];
  	}else{
     $grouporder = "customers_describes = ".$_GET['groupfilter'];
  	}
  }
...
<div style="font-size:12px; background:#fef8af; padding:5px; color:#ff6c00;"><?php echo sprintf(TEXT_MAXIMUM_ENTRIES, MAX_ADDRESS_BOOK_ENTRIES); ?></div>
<div class="clear" style="border-bottom:1px dotted #ddd;" > </div>
<?php
/**
...
?>
<h3 style="margin:0;padding:0;"><?php echo zen_output_string_protected($addresses['firstname'] . ' ' . $addresses['lastname']); ?><?php if ($addresses['address_book_id'] == zen_get_customers_address_primary($_GET['cID'])) echo '&nbsp;' . PRIMARY_ADDRESS ; ?></h3>
<ul style="padding:0px 10px; margin:0; font-size:12px; line-height:130%;"><?php echo zen_address_format($addresses['format_id'], $addresses['address'], true, ' ', '<br />'); ?></ul>

<div class="clear" style="border-bottom:1px dotted #ddd; padding-bottom:5px;"> </div>
<?php } // end list ?>
<div class="pad_10px fr"><?php echo '<a href="' . zen_href_link(FILENAME_CUSTOMERS, 'action=list_addresses_done' . '&cID=' . $_GET['cID'] . ($_GET['page'] > 0 ? '&page=' . $_GET['page'] : ''), 'NONSSL') . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?>
</fieldset>
<?php
        die();
        break;
...
// END newsletter_subscribe mod 3/3
      ...
      default:
        ...
        $customers = $db->Execute("select c.customers_id, c.customers_gender, c.customers_firstname,
                                          ...
                                          a.entry_country_id,a.entry_phone, c.customers_telephone, c.customers_fax,
...
function changegroupfilter(obj, sort_url){
  if(obj.value != '-1'){
    if(sort_url.indexOf('?') > -1){
      window.location.href= sort_url + "&groupfilter=" + obj.value;
    }
    else{
      window.location.href= sort_url + "?groupfilter=" + obj.value;
    }
...
?></td>
          ...
          <tr>
            ...
            <td class="main">
<?php
    echo zen_draw_input_field('entry_phone', $cInfo->entry_phone, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_phone', 15), true);
...
?>
          ...
          <td>
          <?php
           echo '客户搜索'.' '.zen_draw_pull_down_menu('groupfilter', $bestYouArray, (isset($_GET['groupfilter'])?$_GET['groupfilter']:''), 'onchange="changegroupfilter(this,\''.cleanSameArg('groupfilter').'\');" class="select1" rel="dropdown"');
           ?>
          </td>
          </tr>
        </table></td>
...
// Sort Listing
          switch ($_GET['list_order']) {
              ...
              case "describes":
              $disp_order = "c.customers_describes";
              break;
              case "describes-desc":
              $disp_order = "c.customers_describes DESC";
              break;
              case "login-asc":
...
?>
             <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <!--<td class="dataTableHeadingContent" align="left" valign="top"> Modify by fpvok Gold
                  <?php echo (($_GET['list_order']=='company' or $_GET['list_order']=='company-desc') ? '<span class="SortOrderHeader">' . TABLE_HEADING_CUSTOMERS_TYPES . '</span>' : TABLE_HEADING_CUSTOMERS_TYPES); ?><br>
                  <a href="<?php echo zen_href_link(basename($PHP_SELF) . '?list_order=describes', '', 'NONSSL'); ?>"><?php echo ($_GET['list_order']=='describes' ? '<span class="SortOrderHeader">Asc</span>' : '<span class="SortOrderHeaderLink">Asc</b>'); ?></a>&nbsp;
                  <a href="<?php echo zen_href_link(basename($PHP_SELF) . '?list_order=describes-desc', '', 'NONSSL'); ?>"><?php echo ($_GET['list_order']=='describes-desc' ? '<span class="SortOrderHeader">Desc</span>' : '<span class="SortOrderHeaderLink">Desc</b>'); ?></a>
                </td>-->
                 <td class="dataTableHeadingContent" align="left" valign="top">
                  <?php echo (($_GET['list_order']=='email' or $_GET['list_order']=='email-desc') ? '<span class="SortOrderHeader">' . '客户邮件' . '</span>' : '客户邮件'); ?><br>
                  <a href="<?php echo zen_href_link(basename($PHP_SELF) . '?list_order=email', '', 'NONSSL'); ?>"><?php echo ($_GET['list_order']=='email' ? '<span class="SortOrderHeader">Asc</span>' : '<span class="SortOrderHeaderLink">Asc</b>'); ?></a>&nbsp;
                  <a href="<?php echo zen_href_link(basename($PHP_SELF) . '?list_order=email-desc', '', 'NONSSL'); ?>"><?php echo ($_GET['list_order']=='email-desc' ? '<span class="SortOrderHeader">Desc</span>' : '<span class="SortOrderHeaderLink">Desc</b>'); ?></a>
                </td>
                <td class="dataTableHeadingContent" align="left" valign="top">
...
<?php
    ...
    if (isset($_GET['search']) && zen_not_null($_GET['search'])) {
      ...
      $search = " (c.customers_lastname like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%' or c.customers_telephone rlike ':keywords:' or a.entry_company rlike ':keywords:' or a.entry_street_address rlike ':keywords:' or a.entry_city rlike ':keywords:' or a.entry_postcode rlike ':keywords:')";
    ...
    if (isset($_GET['search'])){
	    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_describes, c.customers_firstname, c.customers_email_address, c.customers_group_pricing, a.entry_country_id, a.entry_company, ci.customers_info_date_of_last_logon, ci.customers_info_date_account_created " . $new_fields . ",
	    cgc.amount
	    ...
	    where " . $search . $grouporder . " order by ". $disp_order;
    }elseif (isset($_GET['groupfilter'])){
	    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_describes, c.customers_firstname, c.customers_email_address, c.customers_group_pricing, a.entry_country_id, a.entry_company, ci.customers_info_date_of_last_logon, ci.customers_info_date_account_created " . $new_fields . ",
	    cgc.amount
	    from " . TABLE_CUSTOMERS . " c
	    left join " . TABLE_CUSTOMERS_INFO . " ci on c.customers_id= ci.customers_info_id
	    left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . "
	    left join " . TABLE_COUPON_GV_CUSTOMER . " cgc on c.customers_id = cgc.customer_id 
	    where " . $grouporder  . " order by " .$disp_order;
    }else {
      $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_describes, c.customers_firstname, c.customers_email_address, c.customers_group_pricing, a.entry_country_id, a.entry_company, ci.customers_info_date_of_last_logon, ci.customers_info_date_account_created " . $new_fields . ",
      cgc.amount
      from " . TABLE_CUSTOMERS . " c
      left join " . TABLE_CUSTOMERS_INFO . " ci on c.customers_id= ci.customers_info_id
      left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . "
      left join " . TABLE_COUPON_GV_CUSTOMER . " cgc on c.customers_id = cgc.customer_id  order by " .$disp_order;
    }
// Split Page
...
//    zen_redirect(zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $_GET['cID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'NONSSL'));
    ...
    while (!$customers->EOF) {
      ...
      if ((!isset($_GET['cID']) || (isset($_GET['cID']) && ($_GET['cID'] == $customers->fields['customers_id']))) && !isset($cInfo)) {
        ...
        $customer_info = array_merge($info->fields, $reviews->fields);
...
?>
                ...
                <!--<td class="dataTableContent"><?php echo customerTypes($customers->fields['customers_describes']); ?></td>-->
