...
if (is_file ( DIR_WS_MODULES . 'quick_import.php' ))
	include_once (DIR_WS_MODULES . 'quick_import.php');
...
$sort_by = 'p.products_recommend DESC,p.products_sort_order,p.products_id DESC';
...
while ( ! $classes->EOF ) {
	$tax_class_array [] = array ('id' => $classes->fields ['tax_class_id'], 'text' => $classes->fields ['tax_class_title'] );
	$classes->MoveNext ();
}
// eof get tx classes
...
switch ($_GET ['action']) {
	case 'update' :
		...
		if ($_POST ['quick_updates_new'] ['products_price']) {
			foreach ( $_POST ['quick_updates_new'] ['products_price'] as $products_id => $new_value ) {
				...
				if (($_POST ['quick_updates_new'] ['products_price'] [$products_id] != $_POST ['quick_updates_old'] ['products_price'] [$products_id]) && $apply_price_update) {
					...
					$priceaa = ($_POST ['quick_updates_new'] ['products_price'] [$products_id] - $_POST ['quick_updates_old'] ['products_price'] [$products_id]);
					$db->Execute ( "UPDATE " . TABLE_PRODUCTS . " SET products_price='" . zen_db_input ( $new_value ) . "', products_price_retail=products_price_retail+'" . $priceaa . "', products_price_sample=products_price_sample+'" . $priceaa . "', products_last_modified=now() WHERE products_id=" . ( int ) $products_id );
		...
		if ($_POST ['quick_updates_old'] ['products_freeshipping']) {
			foreach ( $_POST ['quick_updates_old'] ['products_freeshipping'] as $products_id => $freeshipping ) {
				if (! isset ( $_POST ['quick_updates_new'] ['products_freeshipping'] [$products_id] ))
					$_POST ['quick_updates_new'] ['products_freeshipping'] [$products_id] = '0';
				if ($_POST ['quick_updates_new'] ['products_freeshipping'] [$products_id] != $_POST ['quick_updates_old'] ['products_freeshipping'] [$products_id]) {
					$quick_updates_count ['products_freeshipping'] [$products_id] = $products_id;
					zen_set_product_freeshipping ( ( int ) $products_id, ( int ) $_POST ['quick_updates_new'] ['products_freeshipping'] [$products_id] );
				}
			}
		}
		
		if ($_POST ['quick_updates_old'] ['products_recommend']) {			
			foreach ( $_POST ['quick_updates_old'] ['products_recommend'] as $products_id => $pr ) {
				if (! isset ( $_POST ['quick_updates_new'] ['products_recommend'] [$products_id] ))
					$_POST ['quick_updates_new'] ['products_recommend'] [$products_id] = '0';
				if ($_POST ['quick_updates_new'] ['products_recommend'] [$products_id] != $_POST ['quick_updates_old'] ['products_recommend'] [$products_id]) {
					$quick_updates_count ['products_recommend'] [$products_id] = $products_id;
					zen_set_products_recommend ( ( int ) $products_id, ( int ) $_POST ['quick_updates_new'] ['products_recommend'] [$products_id] );
				}
			}
		}
...
if ($sort_by && ! ereg ( 'order by', $sort_by )) {
	...
	$sort_by = 'order by p.products_recommend DESC,p.products_sort_order,' . 'products_id DESC';
}
...
$products_query_raw = "select p.products_id, p.products_type, p.products_image,
                                ...
                                p.products_recommend,pd.products_description,
...
if ($current_category_id > 0) {
	...
	{
		$catIdSring = '(';
		$categories_array = zen_get_categories($categories_array,$current_category_id);
		foreach ($categories_array as $cats)
		{
			$catIdSring .=$cats['id'].',';
		}
		$catIdSring = substr($catIdSring,0,strlen($catIdSring)-1).')';
		$where [] = "p2c.categories_id in " . $catIdSring;
	}
	else 
	{
...
<link rel="stylesheet" type="text/css"
	href="includes/stylesheet_quick_updates.css">
<script language="javascript" src="includes/menu.js"></script>
<script language="javascript" src="includes/general.js"></script>
<script language="javascript"
	src="includes/javascript/quick_updates_price_calculations.js"></script>
...
?>
		...
		<div class="quHeadingText"><?php
		echo QU_HEADING_TEXT;
		?></div>
		<!-- eof pageHeading --> <!-- bof top forms -->
		<table class="quTop">
			<tr>
				<td>
            <?php
				...
				<td class="smallText" align="center" valign="top"><!-- bof spec_price form -->
            <?php
            ...
            <?php
												...
												?>
          <!-- eof spec_price form --></td>
				<td class="smallText" align="center" valign="top"></td>
				<td class="smallText"></td>
				<td class="smallText" align="center" valign="top">
<?php
// bof toggle switch for editor
echo TEXT_EDITOR_INFO . zen_draw_form ( 'set_editor_form', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('action', 'reset_editor' ) ), 'get' ) . '&nbsp;' . zen_draw_pull_down_menu ( 'reset_editor', $editors_pulldown, $current_editor_key, 'onChange="this.form.submit();"' ) . zen_hide_session_id () . zen_draw_hidden_field ( 'action', 'set_editor' ) . '</form>';
...
?>
		...
		<table class="quTop">
			<tr>
				...
				echo zen_draw_form ( 'row_by_page', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('row_by_page' ) ), 'get' ) . TEXT_MAXI_ROW_BY_PAGE . '&nbsp;&nbsp;' . zen_draw_pull_down_menu ( 'row_by_page', $row_bypage_array, $row_by_page, 'onChange="this.form.submit();"' );
				?></form>
				</td>
				<td class="smallText" align="center" valign="top"><?php
				echo zen_draw_form ( 'manufacturers', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('manufacturer' ) ), 'get' ) . DISPLAY_MANUFACTURERS . '&nbsp;&nbsp' . zen_draw_pull_down_menu ( "manufacturer", $manufacturers_array, $manufacturer, 'onChange="this.form.submit();"' );
				?></form>
				</td>
				<td class="smallText" align="center" valign="top"><?php
				echo zen_draw_form ( 'manufacturers', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('manufacturer' ) ), 'get' ) . DISPLAY_STATUS . '&nbsp;&nbsp' . zen_draw_pull_down_menu ( "products_status", array ('0' => array ('id' => 'all', 'text' => 'All' ), '1' => array ('id' => '1', 'text' => 'Active' ), '2' => array ('id' => '0', 'text' => 'Inactive' ) ), $_SESSION ['quick_updates'] ['products_status'], 'onChange="this.form.submit();"' );
				?></form>
				</td>
				<td class="smallText" align="center" valign="top"><?php
				echo zen_draw_form ( 'categorie', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('cPath' ) ), 'get' ) . DISPLAY_CATEGORIES . '&nbsp;&nbsp;' . zen_draw_pull_down_menu ( 'cPath', $quick_updates_category_tree, $current_category_id, 'onChange="this.form.submit();"' ) . '</form>';
				?></td>
				<td>
            <?php
		...
		<!-- eof top forms --> <!-- bof quick_updates form -->
      <?php
						echo zen_draw_form ( 'quick_updates', FILENAME_QUICK_UPDATES, zen_get_all_get_params ( array ('action' ) ) . 'action=update', 'post' );
						?>
      <!-- bof quick_updates form table -->
		<table class="quFormTable" cellspacing="0">
			<tr>
				<td><!-- bof button_update table -->
				<table>
					<tr>
						<td class="smalltext" align="middle"><?php
						echo WARNING_MESSAGE;
						?> </td>
						<td class="pageHeading" align="right"><script
							language="javascript">
				...
				<!-- eof button_update table --> <!-- bof wrapper quickUpdatesProductsTable -->
				<table id="quickUpdatesProductsTable" cellspacing="0">
					<tr>
						<td valign="top"><!-- bof quickUpdates Table -->
						<table cellspacing="0">
							<!-- bof dataTableHeadingRow -->
							<tr class="dataTableHeadingRow">
<?php
//if(QUICKUPDATES_DISPLAY_ID == 'true') // we always display the id
echo zen_quickupdates_table_head ( 'p.products_recommend DESC,p.products_sort_order,p.products_id', TABLE_HEADING_ID );
...
if (QUICKUPDATES_MODIFY_FREESHIPPING == 'true')
	echo zen_quickupdates_table_head ( 'p.product_is_always_free_shipping', TABLE_HEADING_FREESHIPPING );
if (QUICKUPDATES_MODIFY_PRODUCTS_RECOMMEND == 'true')
	echo zen_quickupdates_table_head ( 'p.products_recommend', TABLE_HEADING_PRODUCTS_RECOMMEND );
...
while ( ! $products->EOF ) {
	...
	if ($prev_products_id != $products->fields ['products_id']) {
		...
		if (QUICKUPDATES_DISPLAY_THUMBNAIL == 'true') {
			echo '<td class="smallText productsImage">' . zen_draw_hidden_field ( 'quick_updates_new[products_image][' . $products->fields ['products_id'] . ']', stripslashes ( $products->fields ['products_image'] ), 'id="SelectImageName_' . $products->fields ['products_id'] . '"' ) . zen_draw_hidden_field ( 'quick_updates_old[products_image][' . $products->fields ['products_id'] . ']', stripslashes ( $products->fields ['products_image'] ) ) . '<a href="javascript:selectFileID=\'SelectImageName_' . $products->fields ['products_id'] . '\';updateImgId=\'SelectImageName_' . $products->fields ['products_id'] . '_img\';popupWindow(\'' . zen_href_link ( FILENAME_POPUP_FILE_SELECT, 'sdir=' . dirname ( (is_int ( strpos ( $products->fields ['products_image'], ',' ) ) ? substr ( $products->fields ['products_image'], 0, strpos ( $products->fields ['products_image'], ',' ) ) : $products->fields ['products_image']) ) . '/' ) . '\');">' . zen_image ( DIR_WS_CATALOG . DIR_WS_IMAGES . $products->fields ['products_image'], TEXT_SELECT_IMAGE, QUICKUPDATES_DISPLAY_THUMBNAIL_WIDTH, QUICKUPDATES_DISPLAY_THUMBNAIL_HEIGHT, 'id="SelectImageName_' . $products->fields ['products_id'] . '_img"' ) . '</a>' . '</td>' . "\n";
		}
		...
		if (QUICKUPDATES_MODIFY_STATUS == 'true') {
			echo '<td class="smallText">' . zen_draw_checkbox_field ( 'quick_updates_new[products_status][' . $products->fields ['products_id'] . ']', 1, false, $products->fields ['products_status'], '' ) . zen_draw_hidden_field ( 'quick_updates_old[products_status][' . $products->fields ['products_id'] . ']', $products->fields ['products_status'] ) . '</td>' . "\n";
		}
		
		if (QUICKUPDATES_MODIFY_FREESHIPPING == 'true') {
			echo '<td class="smallText">' . zen_draw_checkbox_field ( 'quick_updates_new[products_freeshipping][' . $products->fields ['products_id'] . ']', 1, false, $products->fields ['product_is_always_free_shipping'], '' ) . zen_draw_hidden_field ( 'quick_updates_old[products_freeshipping][' . $products->fields ['products_id'] . ']', $products->fields ['product_is_always_free_shipping'] ) . '</td>' . "\n";
		}
		
		if (QUICKUPDATES_MODIFY_PRODUCTS_RECOMMEND == 'true') {
			echo '<td class="smallText">' . zen_draw_checkbox_field ( 'quick_updates_new[products_recommend][' . $products->fields ['products_id'] . ']', 1, false, $products->fields ['products_recommend'], '' ) . zen_draw_hidden_field ( 'quick_updates_old[products_recommend][' . $products->fields ['products_id'] . ']', $products->fields ['products_recommend'] ) . '</td>' . "\n";
		}
		
		if (QUICKUPDATES_MODIFY_SORT_ORDER == 'true') {
			echo '<td class="smallText">' . zen_draw_input_field ( 'quick_updates_new[products_sort_order][' . $products->fields ['products_id'] . ']', $products->fields ['products_sort_order'], 'size="3"' ) . zen_draw_hidden_field ( 'quick_updates_old[products_sort_order][' . $products->fields ['products_id'] . ']', $products->fields ['products_sort_order'] ) . '</td>' . "\n";
		}
		...
		if (in_array ( $products->fields ['products_id'], $specials_array )) {
			...
			$flag_special = true;
			echo '<td class="smallText specialPrice" style="display:none;">';
		} else {
			$flag_special = false;
			echo '<td class="smallText productsPrice" style="display:none;">';
		}
...
?>
						...
						<!-- eof quickUpdates Table --></td>
					</tr>
				</table>
				<!-- eof wrapper quickUpdatesProductsTable --></td>
			</tr>
			<tr>
				<td align="right">
<?php
...
?>
		...
		<table class="quBottom">
			<tr>
				<td><!-- bof export viewed products paulm //-->
            <?php
												...
												?>
            <!-- eof export viewed products paulm //--></td>
				<td><!-- // bof quick copy form -->
            <?php
				...
				<!-- // eof quick copy form --></td>
			</tr>
		</table>
		</td>
	</tr>
</table>
<!-- eof #quickUpdatesWrapper -->
