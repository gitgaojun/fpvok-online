...
<?php
    ...
    if (REVIEWS_APPROVAL=='1') {
      $thisStyle = $reviews_pending->fields['count']>0?'style="color:red;font-weight:bold"':'';
	  echo '<div class="row"><span class="left"><a href="' . zen_href_link(FILENAME_REVIEWS, 'status=1', 'NONSSL') . '">' . BOX_ENTRY_REVIEWS_PENDING . '</a></span><span class="rigth" '.$thisStyle.'>' . $reviews_pending->fields['count']. '</span></div>';
    }
...
?>
 ...
 <div class="reportBox">
  ...
  <?php   $orders_contents = '';
  $orders_status = $db->Execute("select orders_status_name, orders_status_id from " . TABLE_ORDERS_STATUS . " where language_id = '" . $_SESSION['languages_id'] . "' order by orders_status_id");
