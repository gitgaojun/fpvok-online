  ...
  if (zen_not_null($action)) {
    switch ($action) {
      case 'setflag':
        ...
        break;
	case 'setflag1':
        zen_set_specials_status1($_GET['id'], $_GET['flag']);
...
// check if priced by attribute
        ...
        $base_price = zen_get_products_base_price($_POST['products_id']);
		$stotl_price = number_format(($base_price-$specials_price)/$specials_price,2)*100;
        ...
        $db->Execute("insert into " . TABLE_SPECIALS . "
                    (products_id, specials_new_products_price,discount, specials_date_added, expires_date, status, specials_date_available)
      ...
      case 'update':
        ...
        $tmp_value = zen_db_prepare_input($_POST['specials_price']);
		$base_price = zen_get_products_base_price($_POST['update_products_id']);
		$stotl_price = number_format(($base_price-$tmp_value)/$base_price,2)*100;
      ...
      // add empty special
		...
		$base_price = zen_get_products_base_price($products_id);
		$stotl_price = number_format(($base_price-zen_db_input($specials_price))/$base_price,2)*100;
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
         <tr><?php echo zen_draw_form('search8', 'specials.php', '', 'get'); ?>
...
// show reset search
  ...
  echo HEADING_TITLE_SEARCH_DETAIL . ' ' . zen_draw_input_field('search8') . zen_hide_session_id();
  if (isset($_GET['search']) && zen_not_null($_GET['search'])) {
      ...
      <tr><form name="new_special" <?php echo 'action="' . zen_href_link(FILENAME_SPECIALS, zen_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action . '&go_back=' . $_GET['go_back'], 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo zen_draw_hidden_field('specials_id', $_GET['sID']); ?>
        <td><br><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main"><select style="font-size:10px" size="5" name="products_id">
            <?php 
  if (strlen($_GET['search8'])>0) {
		
// only show $current_categories_id
      $products_t = "select p.products_id, pd.products_name, p.products_price, p.products_model, ptc.categories_id
                                from " . TABLE_PRODUCTS . " p
                                left join " . TABLE_PRODUCTS_TO_CATEGORIES . " ptc on ptc.products_id = p.products_id, " .
                                TABLE_PRODUCTS_DESCRIPTION . " pd
                                where p.products_id = pd.products_id
                                and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
                                and p.products_id = " . (int)$_GET['search8'] . "
                                order by products_name";
    } else {
      $products_t = "select p.products_id, pd.products_name, p.products_price, p.products_model
                                from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd
                                where p.products_id = pd.products_id
                                and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
                                order by products_name";
	    ...
	    $products = $db->Execute($products_t);
		while (!$products->EOF) {	
		$display_price = zen_get_products_base_price($products->fields['products_id']);
			?>
            <option value="<?php echo $products->fields['products_id']; ?>"><?php echo $products->fields['products_name'] . ' (' . $currencies->format($display_price) . ')' . ' [' . $products->fields['products_model'] . '] ' . ' - ID# ' . $products->fields['products_id'] ; ?></option>
     <?php 
	 $products->MoveNext();
      } 
		 ?>   
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="center">推荐</td>
...
?>
                ...
                <td  class="dataTableContent" align="center">
<?php
      if ($specials->fields['status1'] == '1') {
        echo '<a href="' . zen_href_link(FILENAME_SPECIALS, 'action=setflag1&flag=0&id=' . $specials->fields['specials_id'] . '&page=' . $_GET['page'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_green_on.gif', IMAGE_ICON_STATUS_ON) . '</a>';
      } else {
        echo '<a href="' . zen_href_link(FILENAME_SPECIALS, 'action=setflag1&flag=1&id=' . $specials->fields['specials_id'] . '&page=' . $_GET['page'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_red_on.gif', IMAGE_ICON_STATUS_OFF) . '</a>';
      }
?>
                </td>
