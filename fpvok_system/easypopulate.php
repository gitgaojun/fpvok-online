...
if (zen_not_null($ep_dltype)) {
	...
	case 'full':
		...
		foreach ($langcode as $key => $lang){
			...
			$filelayout[] = 'v_products_small_description_' . $l_id;
		...
		$filelayout[] = 'v_products_price_retail';
		...
		*/
		...
		$filelayout_sql = "SELECT
			...
			p.products_price_retail as v_products_price_retail,
	...
	case 'froogle':
		...
		$filetemp['small_description'] = 'v_froogle_products_small_description_1';
		$filetemp['price'] = 'v_products_price';
		$filetemp['price_retail'] = 'v_products_price_retail';
...
//		$row['v_froogle_product_id']    = $row['v_products_model'];
		...
		foreach ($langcode as $key => $lang){
			...
			$row['v_products_small_description_' . $lid]  = $row2['products_small_description'];
			if ($ep_supported_mods['psd'] == true) {
			...
			$row['v_froogle_products_small_description_' . $lid] = '"' . html_entity_decode(strip_tags(str_replace('"','""',$row2['products_small_description']))) . '"';
			/*
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			*/
			...
			foreach ($langcode as $key => $lang){
				...
				$row['v_products_small_description_' . $lang['id']]   = $row2['products_small_description'];// description assigned
		...
		*/
		...
		foreach ($langcode as $lang){
			...
			if (isset($filelayout['v_products_name_' . $l_id ])){ // do for each language in our upload file if exist
				...
				$v_products_small_description[$l_id] = $items[$filelayout['v_products_small_description_' . $l_id ]];
				// if short descriptions exist
				if ($ep_supported_mods['psd'] == true) {
			...
			if ( $row = mysql_fetch_array($result) ) {
				...
				*/
				...
				$query = "UPDATE " . TABLE_PRODUCTS . " SET
						...
						products_price_retail					=	'" . zen_db_input($v_products_price_retail)."' ,
			...
			if ($name != ''){
					...
					if (mysql_num_rows($result) == 0) {
						...
						$sql ="INSERT INTO ".TABLE_PRODUCTS_DESCRIPTION." SET
									...
									products_small_description	=	'".zen_db_input($v_products_small_description[$key])."',
									".$ep_supported_mods_sql."
							...
							$sql =
								"INSERT INTO ".TABLE_PRODUCTS_DESCRIPTION."
									...
									VALUES (
										...
										'" . zen_db_input($v_products_small_description[$key]) . "',
										'". $v_products_url[$key] . "',
