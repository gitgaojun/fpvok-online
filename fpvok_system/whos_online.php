...
?>
<?php 
/*$str="<div class=\"notice_1\">
<ul class=\"mycon\" style=\"font-size: 12px;\">
<div style=\"font-size: 18px; font-weight: bold;\">Unlocked for Worldwide Use</div>
<br /><strong>This Cell Phone works with the following GSM Frequencies: <span style=\"color: #ea0212;\"><span>850/900/</span><span>1800/1900 </span></span><span>MHz.</span></strong><br />Always confirm with your carriers or providers before buying this item. To check the specific <strong>network frequency</strong> for your <strong>carrier</strong> and <strong>region</strong>&nbsp;<button onclick=\"window.open('http://www.gsmworld.com/roaming/gsminfo/index.shtml')\"><span style=\"font-weight: bold; font-size: 12px; color: #204f13;\">Click here</span></button> <br /></ul><em></em></div>";

	$flash_page_query = "select p.products_id,p.products_image,pd.products_name,pd.products_description from " . TABLE_PRODUCTS ." p, ". TABLE_PRODUCTS_DESCRIPTION . " pd where p.`products_id`=pd.`products_id` AND p.master_categories_id in(407,408,409,410,411,412,414)";
	
          $new_special_insert = $db->Execute($flash_page_query );
					  
		while (!$new_special_insert->EOF) {	
		$pages_title=zen_db_prepare_input($str).zen_db_prepare_input($new_special_insert->fields['products_description']);
          $sql_data_array = array('products_description' => $pages_title);
       
								  
		zen_db_perform('products_description', $sql_data_array, 'update', "products_id = '" . (int)$new_special_insert->fields['products_id'] . "'");
$new_special_insert->MoveNext();
}*/
?>
