  ...
  if (zen_not_null($action)) {
    ...
    switch ($action) {
      ...
      case 'add_product_options':
		...
		$products_options_catog = $_POST['products_options_catog'];

        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          $option_name = zen_db_prepare_input($option_name_array[$languages[$i]['id']]);

          $db->Execute("insert into " . TABLE_PRODUCTS_OPTIONS . "
                      (products_options_id, products_options_name, language_id, products_options_sort_order, products_options_type, products_options_images_per_row, products_options_images_style, products_options_rows, products_options_catog)
...
//          zen_db_query("update " . TABLE_PRODUCTS_OPTIONS . " set products_options_name = '" . zen_db_input($option_name) . "', products_options_type = '" . $option_type . "' where products_options_id = '" . (int)$option_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
          ...
          $db->Execute("update " . TABLE_PRODUCTS_OPTIONS . "
                        set products_options_name = '" . zen_db_input($option_name) . "', products_options_type = '" . $option_type . "', products_options_length = '" . zen_db_input($products_options_length) . "', products_options_comment = '" . zen_db_input($products_options_comment) . "', products_options_size = '" . zen_db_input($products_options_size) . "', products_options_sort_order = '" . zen_db_input($products_options_sort_order) . "', products_options_images_per_row = '" . zen_db_input($products_options_images_per_row) . "', products_options_images_style = '" . zen_db_input($products_options_images_style) . "', products_options_rows = '" . zen_db_input($products_options_rows) . "', products_options_catog = '" . zen_db_input($products_options_catog) . "'
...
?>
              ...
              <tr>
                <td colspan="8"><?php echo zen_black_line(); ?></td>
              </tr>
              <tr class="dataTableHeadingRow">
				...
				<td class="dataTableHeadingContent">&nbsp;左侧筛选&nbsp;</td>
...
// edit option name
      if (($action == 'update_option') && ($_GET['option_id'] == $options_values->fields['products_options_id'])) {
        ...
        $inputs2 = '';
        for ($i = 0, $n = sizeof($languages); $i < $n; $i ++) {
          $option_name = $db->Execute("select products_options_name, products_options_sort_order, products_options_size, products_options_length, products_options_comment, products_options_images_per_row, products_options_images_style, products_options_rows, products_options_catog
                                       from " . TABLE_PRODUCTS_OPTIONS . "
...
?>
				...
				<select rel="dropdown" name="products_options_catog">
				<option value="1" <?php echo ($options_values->fields['products_options_type']==1 ? 'selected="selected"' :'') ?>>是</option>
				<option value="0" <?php echo ($options_values->fields['products_options_type']==1 ? 'selected="selected"' :'') ?>>否</option>
				</select></td>
      ...
      <tr>
        <td colspan="8"><?php echo zen_draw_separator('pixel_black.gif', '100%', '2'); ?></td>
      </tr>

              <tr class="attributeBoxContent">
                <td class="attributeBoxContent">&nbsp;</td>
                <td colspan="7" class="attributeBoxContent"><?php echo TEXT_OPTION_ATTIBUTE_MAX_LENGTH . $inputs2; ?></td>
              </tr>
              <tr class="attributeBoxContent">
                <td class="attributeBoxContent">&nbsp;</td>
                <td colspan="7" class="attributeBoxContent">
              <?php echo '<br />' .
...
?>
                ...
                <td class="smallText">&nbsp;<?php echo ($options_values->fields["products_options_catog"]==1?'Yes':'No'); ?>&nbsp;</td>
...
?>
                ...
                <td colspan="2" class="smallText">分类显示:
				
				<select name="products_options_catog" rel="dropdown">
