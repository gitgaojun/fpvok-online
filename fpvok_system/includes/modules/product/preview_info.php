  ...
  die('Illegal Access');
    ...
    if (zen_not_null($_POST)) {
      ...
      $product = $db->Execute("select p.products_id, pd.language_id, pd.products_name,
                                      pd.products_description, pd.products_description1, pd.products_description2, pd.products_url, p.products_quantity,
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
		...
		$pInfo->products_description1 = zen_get_products_description1($pInfo->products_id, $languages[$i]['id']);
		$pInfo->products_description2 = zen_get_products_description2($pInfo->products_id, $languages[$i]['id']);
		...
		$pInfo->products_description1 = zen_db_prepare_input($products_description1[$languages[$i]['id']]);
...
/* Re-Post all POST'ed variables */
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		...
		echo zen_draw_hidden_field('products_description1[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_description1[$languages[$i]['id']])));
        ...
        echo zen_draw_hidden_field ( 'products_image', stripslashes ( $products_image_name_string ) );
		echo zen_draw_hidden_field ( 'values_id', stripslashes ( join(",",$_POST['values_id'])) );
