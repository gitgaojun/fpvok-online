...
'products_image' => '',
'products_price' => '',
'products_price_retail' => '',
'products_price_sample' => '',
...
'products_price_retail' => '',
'product_is_wholesale' => '',
'product_wholesale_min' => '',
...
if (isset($_GET['pID']) && empty($_POST)) {
	$product = $db->Execute("select pd.products_name, pd.products_small_description, pd.products_description, pd.products_description1, pd.products_description2, pd.products_url,p.accessories_product,p.products_price_dapei,
                                      p.products_price_retail,p.products_price_sample,p.product_is_wholesale,p.product_wholesale_min,
	...
	$products_small_description = $_POST['products_small_description'];
	$products_description = $_POST['products_description'];
	$products_description1 = $_POST['products_description1'];
...
// Product is Free
if (!isset($pInfo->product_is_wholesale)) $pInfo->product_is_wholesale = '0';
switch ($pInfo->product_is_wholesale) {
	case '0': $in_product_is_wholesale = false; $out_product_is_wholesale = true; break;
	case '1': $in_product_is_wholesale = true; $out_product_is_wholesale = false; break;
	default: $in_product_is_wholesale = false; $out_product_is_wholesale = true;
}
...
function updateSampleGross() {
	var taxRate = getTaxRate();
	var grossValue = document.forms["new_product"].products_price_sample.value;

	if (taxRate > 0) {
		grossValue = grossValue * ((taxRate / 100) + 1);
	}

	document.forms["new_product"].products_price_sample_gross.value = doRound(grossValue, 4);
}

function updateSampleNet() {
	var taxRate = getTaxRate();
	var netValue = document.forms["new_product"].products_price_sample_gross.value;

	if (taxRate > 0) {
		netValue = netValue / ((taxRate / 100) + 1);
	}

	document.forms["new_product"].products_price_sample.value = doRound(netValue, 4);
}

function updateRetailGross() {
	var taxRate = getTaxRate();
	var grossValue = document.forms["new_product"].products_price_retail.value;

	if (taxRate > 0) {
		grossValue = grossValue * ((taxRate / 100) + 1);
	}

	document.forms["new_product"].products_price_retail_gross.value = doRound(grossValue, 4);
}

function updateRetailNet() {
	var taxRate = getTaxRate();
	var netValue = document.forms["new_product"].products_price_retail_gross.value;

	if (taxRate > 0) {
		netValue = netValue / ((taxRate / 100) + 1);
	}

	document.forms["new_product"].products_price_retail.value = doRound(netValue, 4);
}
...
if (zen_get_product_is_linked($_GET['pID']) == 'true' and $_GET['pID'] > 0) {
	...
	<tr bgcolor="#599659">
	<td class="main"><b><font color = #FFFFFF>Product Information:</font></b></td>
	</tr>
	<tr>
	...
	for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		...
		<td class="main"><?php echo zen_draw_input_field('products_name[' . $languages[$i]['id'] . ']', (isset($products_name[$languages[$i]['id']]) ? stripslashes($products_name[$languages[$i]['id']]) : zen_get_products_name($pInfo->products_id, $languages[$i]['id'])), ' size="100"'); ?></td>
		</tr>
		<?php
	}
	?>
    ...
    <tr>
	<td class="main">配件产品ID:</td>
	<td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('accessories_product', $pInfo->accessories_product, zen_set_field_length(TABLE_PRODUCTS, 'accessories_product')); ?> (*eg: 34#58#95)</td>
	</tr>
<tr>
	<td colspan="2">
    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="1">
  <tr>
    <td width="22%" height="24" style="font-size:14px"><b>属 性</b></td>
    <td width="78%">&nbsp;</td>
  </tr>
  
<?php

$sql2="select op.products_options_id,op.products_options_name from products_options op where op.language_id = '" . (int)$_SESSION['languages_id'] . "' order by op.products_options_sort_order asc";

$options_type = $db->Execute($sql2); 
$atrr='';$atrr2='';
if(isset($_GET['pID']) && isset($_GET['pID'])>0){
	$color_query_c = "SELECT options_values_id,options_id FROM " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id ='".(int)$_GET['pID']."'";
	$atrr_raw = $db->Execute($color_query_c);
	
	while (!$atrr_raw->EOF) {
	$atrr.='['.$atrr_raw->fields['options_values_id'].']';	
	$atrr2.='['.$atrr_raw->fields['options_id'].']';	
	$atrr_raw->MoveNext();
	}
...
while (!$options_type->EOF) {
 ...
 $sql="select pvp.products_options_id,op.products_options_values_id,op.products_options_values_name from products_options_values op,products_options_values_to_products_options pvp where op.products_options_values_id = pvp.products_options_values_id and pvp.products_options_id='".$options_type->fields['products_options_id']."' and op.language_id = '" . (int)$_SESSION['languages_id'] . "' order by op.products_options_values_sort_order asc";
 
$options_values = $db->Execute($sql);  

 ?>  
  <tr>
    <td height="23" align="right" valign="top" bgcolor="#F5F5F5"><strong><?php echo $options_type->fields['products_options_name'] ?>:</strong>&nbsp;&nbsp;</td>
    <td valign="top" bgcolor="#FDFDFD" id="type_<?php echo $options_type->fields['products_options_id'] ?>">
<?php while (!$options_values->EOF) {?>	
<?php if($options_values->fields['products_options_values_id']==0){?>
<input name="names_id[]"  id="names_id[]" <?php echo (strlen($atrr2)>0 && strstr($atrr2,'['.$options_values->fields['products_options_id'].']')?'checked="checked"':'') ?> type="checkbox" value="<?php echo $options_values->fields['products_options_id'] ?>" /><?php echo $options_values->fields['products_options_values_name'] ?>&nbsp;&nbsp;&nbsp;&nbsp;

<?php }else{?>
<input name="values_id[]"  id="values_id[]" <?php echo (strlen($atrr)>0 && strstr($atrr,'['.$options_values->fields['products_options_values_id'].']')?'checked="checked"':'') ?> type="checkbox" value="<?php echo $options_values->fields['products_options_values_id'] ?>" /><?php echo $options_values->fields['products_options_values_name'] ?>&nbsp;&nbsp;&nbsp;&nbsp;
<?php }?>
<?php
...
?>
	...
	<td class="main"><?php echo TEXT_PRODUCTS_DATE_AVAILABLE; ?><br /><small>(YYYY-MM-DD)</small></td>
		  ...
		  ?>
          <tr>
            <td class="main"><?php if ($i == 0) echo TEXT_PRODUCTS_SMALL_DESCRIPTION; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_small_description',$pInfo->products_small_description,'size="100"');?></td>
          </tr>
          <?php 
          }
          ?>
...
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0) echo TEXT_PRODUCTS_DESCRIPTION1; ?></td>
            <td colspan="2"><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" width="25" valign="top"><?php //echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main" width="100%">
        <?php if ($_SESSION['html_editor_preference_status']=="FCKEDITOR") {
        	$oFCKeditor = new FCKeditor('products_description1[' . $languages[$i]['id'] . ']') ;
        	$oFCKeditor->Value = (isset($products_description1[$languages[$i]['id']])) ? stripslashes($products_description1[$languages[$i]['id']]) : zen_get_products_description1($pInfo->products_id, $languages[$i]['id']) ;
        	$oFCKeditor->Width  = '99%' ;
        	$oFCKeditor->Height = '350' ;
        	//                $oFCKeditor->Config['ToolbarLocation'] = 'Out:xToolbar' ;
        	//                $oFCKeditor->Create() ;
        	...
        	echo zen_draw_textarea_field('products_description1[' . $languages[$i]['id'] . ']', 'soft', '100%', '30', (isset($products_description1[$languages[$i]['id']])) ? stripslashes($products_description1[$languages[$i]['id']]) : zen_get_products_description1($pInfo->products_id, $languages[$i]['id'])); //,'id="'.'products_description' . $languages[$i]['id'] . '"');
          } ?>
        </td>
              </tr>
            </table></td>
...
?>
          ...
          <tr bgcolor="#599659">
            <td class="main"><b><font color = #FFFFFF>Product Image Information:</font></b></td>
			</tr> 
          <tr>
            <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr> 
            ...
            <table width="100%" class="box" summary="Upload" cellspacing="4"
          cellpadding="4">
          <thead>
            <tr>
              <th align="left">Uploads attachments</th>
            </tr>
          </thead>
          <tbody id="attachbodyhidden">
            ...
            <?php
            if (is_int ( strpos ( $pInfo->products_image, ',' ) )) {
            	$imgsrcs = split ( ',', $pInfo->products_image );
            	for($i = 0; $i < count ( $imgsrcs ); $i ++) {
            		$imgsrcs_image_extension[$i] = substr($imgsrcs[$i], strrpos($imgsrcs[$i], '.'));
            		$imgsrcs_base[$i] = substr_replace(substr($imgsrcs[$i], 0, strrpos($imgsrcs[$i], '.')),'l/',0,2);
            		$imgsrcs_v_base[$i] = substr_replace(substr($imgsrcs[$i], 0, strrpos($imgsrcs[$i], '.')),'v/',0,2);
            		$imgsrcs_medium[$i] = '../'.DIR_WS_IMAGES . $imgsrcs_base[$i] . $imgsrcs_image_extension[$i];
            		$imgsrcs_v[$i] = '../'.DIR_WS_IMAGES . $imgsrcs_v_base[$i] . $imgsrcs_image_extension[$i];
            		echo '<div class="list">' . zen_image ($imgsrcs_medium[$i], $pInfo->products_name,'','','','yes');
            		echo '<a class="delButton" onclick="this.parentNode.parentNode.removeChild( this.parentNode);">Delete</a><input type="hidden" name="existimg_'.$i.'" value ="'.$imgsrcs[$i].'" /></div>';
            	}
            }else{
            	if( $pInfo->products_image == ""){
            		//nothing
            	}else{
            		$imgsrc_extension = substr($pInfo->products_image, strrpos($pInfo->products_image, '.'));
            		$imgsrc_image_base = substr_replace(substr($pInfo->products_image, 0, strrpos($pInfo->products_image, '.')),'l/',0,2);
            		$imgsrc_medium = '../'.DIR_WS_IMAGES . $imgsrc_image_base . $imgsrc_extension;
            		echo '<div class="list"><a href="'.$imgsrc_medium.'">' . zen_image ($imgsrc_medium, $pInfo->products_name,'','','','yes').'</a>';
            		echo '<a class="delButton" onclick=" this.parentNode.parentNode.removeChild( this.parentNode);">Delete</a><input type="hidden" name="existimg_0" value ="'.$pInfo->products_image.'" /></div>';
            	}
            }
            ?>
            ...
            <tr>
              <td align="left" width="230"><input id="my_file_element" type="file" name="file_1"></td>
            </tr>
            <tr>
              <td><!-- This is where the output will appear --> <script>
          ...
          <tr bgcolor="#599659">
            <td class="main"><b><font color = #FFFFFF>Product Pricing Information:</font></b></td>
			    </tr> 
          <tr>
          ...
          <tr>
            <td class="main"><?php echo TEXT_WHOLESALE_ONLY; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_radio_field('product_wholesale_only', '1', ($in_product_is_wholesale==1)) . '&nbsp;' . TEXT_YES . '&nbsp;&nbsp;' . zen_draw_radio_field('product_wholesale_only', '0', ($in_product_is_wholesale==0)) . '&nbsp;' . TEXT_NO; ?></td>
          </tr>
          ...
          <tr bgcolor="#F4F3F5">
            <td class="main"><?php echo TEXT_PRICE_RETAIL_NET; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_price_retail', $pInfo->products_price_retail, 'onKeyUp="updateRetailGross()"'); ?></td>
          </tr>
          
          <tr bgcolor="#F4F3F5">
            <td class="main"><?php echo TEXT_PRICE_RETAIL_GROSS; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_price_retail_gross', $pInfo->products_price_retail, 'OnKeyUp="updateRetailNet()"'); ?></td>
          </tr>
          
          <tr bgcolor="#F4F3F5">
            <td class="main"><?php echo TEXT_PRODUCTS_SAMPLE_PRICE_NET; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_price_sample', $pInfo->products_price_sample, 'onKeyUp="updateSampleGross()"'); ?></td>
          </tr>
          <tr bgcolor="#F4F3F5">
            <td class="main"><?php echo TEXT_PRODUCTS_SAMPLE_PRICE_GROSS; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_price_sample_gross', $pInfo->products_price_sample, 'OnKeyUp="updateSampleNet()"'); ?></td>
          </tr>
         ...
         <tr bgcolor="#599659">
            <td class="main"><b><font color = #FFFFFF>产品搭配价格:</font></b></td>
			</tr> 
            <tr>
	<td class="main">此产品当作配件的价格:</td>
	<td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_price_dapei', $pInfo->products_price_dapei, zen_set_field_length(TABLE_PRODUCTS, 'products_price_dapei')); ?>(0代表不作为配件) </td>
	</tr>
          <tr bgcolor="#599659">
            <td class="main"><b><font color = #FFFFFF>Product Shipping Information:</font></b></td>
			</tr> 
          <tr>
            <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          ...
          <tr bgcolor="#599659">
            <td class="main"><b><font color = #FFFFFF>Product Quantity Information:</font></b></td>
			</tr> 
          <tr>
            <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
