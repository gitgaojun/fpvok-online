...
<?php
    ...
    if (isset($_GET['search'])) {
      ...
      $categories = $db->Execute("select c.categories_id, cd.categories_name, cd.categories_description, c.categories_image,
                                         c.parent_id, c.sort_order, c.date_added, c.last_modified,c.categories_banner_1_img,c.categories_banner_2_img,
                                         c.categories_banner_1_link,c.categories_banner_2_link,c.categories_type,c.categories_display_type,c.categories_list_types,
                                         c.categories_status
...
<?php
    ...
    switch ($_SESSION['categories_products_sort_order']) {
      case (0):
        ...
        break;
      case (1):
        $order_by = " order by pd.products_id desc";
        break;
      case (2);
    ...
    if (isset($_GET['search'])) {
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                ...
                                or p.products_id like '%" . zen_db_input($_GET['search']) . "%')" .
                                $order_by);
    } else {
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                       p.products_image, p.products_price, p.products_date_added,
