...
// error on 1969 only allows for leap year
  ...
  function zen_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $category_has_products = false, $limit = false) {
    ...
    while (!$categories->EOF) {
      ...
      if ($exclude != $categories->fields['categories_id'] && category_allowed($categories->fields['categories_id']) == 'true') $category_tree_array[] = array('id' => $categories->fields['categories_id'], 'text' => $spacing . $categories->fields['categories_name'] . $mark);
...
// only show $current_categories_id
    ...
    while (!$products->EOF) {
      if (!in_array($products->fields['products_id'], $exclude) && category_allowed(zen_get_products_category_id($products->fields['products_id'])) == 'true') {
...
*/
  ...
  function zen_get_category_info_description($category_id, $language_id) {
    global $db;
    $category = $db->Execute("select product_info_text
                              from " . TABLE_CATEGORIES_DESCRIPTION . "
                              where categories_id = '" . (int)$category_id . "'
                              and language_id = '" . (int)$language_id . "'");

    return $category->fields['product_info_text'];
  }
  ...
  function zen_get_products_small_description($product_id, $language_id) {
    global $db;
    $product = $db->Execute("select products_small_description
                             from " . TABLE_PRODUCTS_DESCRIPTION . "
                             where products_id = '" . (int)$product_id . "'
                             and language_id = '" . (int)$language_id . "'");

    return $product->fields['products_small_description'];
  }
  ...
  function zen_get_products_description1($product_id, $language_id) {
    global $db;
    $product = $db->Execute("select products_description1
                             from " . TABLE_PRODUCTS_DESCRIPTION . "
                             where products_id = '" . (int)$product_id . "'
                             and language_id = '" . (int)$language_id . "'");

    return $product->fields['products_description1'];
  }
...
// TABLES: products, products_to_categories, categories
  function zen_products_in_category_count($categories_id, $include_deactivated = false, $include_child = true, $limit = false) {
    ...
    if ($include_child && category_allowed($categories_id) == 'true') {
...
////
// Sets the product as a recommendation product
  function zen_set_products_recommend($products_id, $products_recommend) {
    global $db;
    if ($products_recommend == '1') {
      return $db->Execute("update " . TABLE_PRODUCTS . "
                           set products_recommend = 1, products_last_modified = now()
                           where products_id = '" . (int)$products_id . "'");

    } elseif ($products_recommend == '0') {
      return $db->Execute("update " . TABLE_PRODUCTS . "
                           set products_recommend = 0, products_last_modified = now()
                           where products_id = '" . (int)$products_id . "'");
    } else {
      return -1;
    }
...
////
// Sets the product as a freeshipping product
  function zen_set_product_freeshipping($products_id, $freeshipping) {
    global $db;
    if ($freeshipping == '1') {
      return $db->Execute("update " . TABLE_PRODUCTS . "
...
//        $calculated_category_path_string .= $calculated_category_path[$i][$j]['text'] . '&nbsp;&gt;&nbsp;';
  ...
  function zen_remove_category($category_id) {
    ...
    // BOF - Admin Profiles for Categories
    $db->Execute("delete from " . TABLE_ADMIN_ALLOWED_CATEGORIES . "
                  where categories_id = '" . (int)$category_id . "'
                  and admin_id = '" . (int)$_SESSION['admin_id'] . "'");
    // EOF - Admin Profiles for Categories
  }
...
// categories pulldown with products
  function zen_draw_products_pull_down_categories($name, $parameters = '', $exclude = '', $show_id = false, $show_parent = false) {
    ...
    while (!$categories->EOF) {
      if (!in_array($categories->fields['categories_id'], $exclude) && category_allowed($categories->fields['categories_id']) == 'true') {
        if ($show_parent == true) {
...
// Sets the status of a product review
  function zen_set_reviews_status($review_id, $status) {
    ...
    function zen_set_asks_status($review_id, $status) {
    global $db;
    if ($status == '1') {
      return $db->Execute("update " . TABLE_ASK . "
                           set status = 1
                           where reviews_id = '" . (int)$review_id . "'");
...
////
// Sets the status of a product review
  function zen_set_reviews_featured($review_id, $featured) {
    global $db;
    if ($featured == '1') {
      return $db->Execute("update " . TABLE_REVIEWS . "
                           set reviews_is_featured = 1
                           where reviews_id = '" . (int)$review_id . "'");

    } elseif ($featured == '0') {
      return $db->Execute("update " . TABLE_REVIEWS . "
                           set reviews_is_featured = 0
                           where reviews_id = '" . (int)$review_id . "'");
...
// set the products_price_sorter
  ...
  function zen_update_products_models($products_id){
  	global $db;
  	$db->Execute("update " . TABLE_PRODUCTS . " set products_model='".$products_id."' where products_id='".$products_id."'");
  }
...
////
  function zen_get_categories($categories_array = '', $parent_id = '0', $indent = '') {
    ...
    while (!$categories->EOF) {
      if (category_allowed($categories->fields['categories_id']) == 'true') {
...
// master category selection
  function zen_get_master_categories_pulldown($product_id) {
    ...
    while (!$master_categories_query->EOF) {
      if (category_allowed($master_categories_query->fields['categories_id']) == 'true') {
...
// Recursive algorithim to restrict all sub_categories to a rpoduct type
  function zen_restrict_sub_categories($zf_cat_id, $zf_type) {
    ...
    while (!$zq_sub_cats->EOF) {
      if (category_allowed($zq_sub_cats->fields['categories_id']) == 'true') {
