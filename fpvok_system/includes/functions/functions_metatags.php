 ...
 */
  ...
  function zen_get_metatags_bottom_keywords($product_id, $language_id) {
    global $db;
    $product = $db->Execute("select metatags_bottom_keywords
                             from " . TABLE_META_TAGS_PRODUCTS_DESCRIPTION . "
                             where products_id = '" . (int)$product_id . "'
                             and language_id = '" . (int)$language_id . "'");

    return $product->fields['metatags_bottom_keywords'];
  }
