...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      echo '          <tr class="dataTableRow">' . "\n" .
           '            <td class="dataTableContent" valign="top">' .zen_info_image(substr(zen_get_products_image($order->products[$i]['id']),0,strpos(zen_get_products_image($order->products[$i]['id']),',')), $order->products[$i]['name'], 30, 30) . '</td>' . "\n" .
           '            <td class="dataTableContent" valign="top" align="right" style="font-weight:bold;color:red">' . $order->products[$i]['qty'] . '&nbsp;x</td>' . "\n" .
