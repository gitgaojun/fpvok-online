...
if (isset ( $_GET ['debug'] ) && ($_GET ['debug'] == 'ON' || $_GET ['debug'] > 0))
	$debug = 'ON';
...
if ($php_disabled_functions = @ini_get ( "disable_functions" )) {
	if ($debug == 'ON')
		$messageStack->add ( 'PHP-Disabled-functions: ' . $php_disabled_functions, 'error' );
...
if ($debug == 'ON')
	$messageStack->add_session ( 'mysqlexe=' . $mysql_exe . '<br />', 'caution' );
if ($debug == 'ON')
	$messageStack->add_session ( 'mysqldumpexe=' . $mysqldump_exe . '<br /><br />', 'caution' );
...
if (zen_not_null ( $action )) {
	switch ($action) {
		...
		case 'backupnow' :
			...
			if ($debug == 'ON' || (zen_not_null ( $dump_results ) && $dump_results != '0'))
			...
			foreach ( $output as $key => $value ) {
				$messageStack->add_session ( "$key => $value<br />", 'caution' );
			}
		...
		case 'restorelocalnow' :
			...
			if (file_exists ( $restore_from ) && $specified_restore_file != '') {
				...
				if ($debug == 'ON')
					$messageStack->add_session ( 'COMMAND: ' . OS_DELIM . $toolfilename . ' ' . $load_params . OS_DELIM, 'caution' );
				...
				if ($debug == 'ON')
					$messageStack->add_session ( "valueB: " . $strB, 'error' );
				if ($debug == 'ON' || (zen_not_null ( $load_results ) && $load_results != '0'))
					$messageStack->add_session ( 'Result code: ' . $load_results, 'caution' );
...
<?php
if (ENABLE_SSL_ADMIN != 'true') { // display security warning about downloads if not SSL 	?>
          <tr>
						<td class="main"><?php
...
while ( $file = $dir->read () ) {
	if (! is_dir ( DIR_FS_BACKUP . $file )) {
		if ($file != '.empty' && $file != 'empty.txt' && $file != 'index.php' && $file != 'index.htm' && $file != 'index.html') {
			$getFileTime = date ( PHP_DATE_TIME_FORMAT, filemtime ( DIR_FS_BACKUP . $file ) );
			$contents [$file] = $getFileTime;
		}
	}
}
arsort ( $contents );
//echo"<pre>";print_r($contents);echo"</pre>";
foreach($contents as $fileName=>$fileTime) {
	$entry = $fileName;
...
<!--                 <td class="dataTableContent" onclick="document.location.href='<?php
	...
	?>'"><?php
	echo '<a href="' . zen_href_link ( FILENAME_BACKUP_MYSQL, 'action=download&file=' . $entry ) . '">' . zen_image ( DIR_WS_ICONS . 'file_download.gif', ICON_FILE_DOWNLOAD ) . '</a>&nbsp;' . $entry;
	?></td> -->
...
?>
              <tr>
                    ...
                    <?php
																				if (($action != 'backup') && (isset ( $dir )) && ! get_cfg_var ( 'safe_mode' ) && $dir_ok == true) {
																					echo '<a href="' . zen_href_link ( FILENAME_BACKUP_MYSQL, 'action=backup' . (($debug == 'ON') ? '&debug=ON' : '') ) . (($tables_to_export != '') ? '&tables=' . str_replace ( ' ', ',', $tables_to_export ) : '') . '">' . zen_image_button ( 'button_backup.gif', IMAGE_BACKUP ) . '</a>&nbsp;&nbsp;';
																				}
																				if (($action != 'restorelocal') && isset ( $dir )) {
																					echo '<a href="' . zen_href_link ( FILENAME_BACKUP_MYSQL, 'action=restorelocal' . (($debug == 'ON') ? '&debug=ON' : '') ) . '">' . zen_image_button ( 'button_restore.gif', IMAGE_RESTORE ) . '</a>';
																				}
																				?>
                </td>
							</tr>
