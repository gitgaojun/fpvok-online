...
/*
  ...
  Version History
  ---------------------------------------------------------------
  Many Changes happened since 11/4/04 -- See changelog in readme file.
...
*/
  ...
  $orders_status_query = $db -> Execute("select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)$_SESSION['languages_id'] . "' order by orders_status_id");
...
','<br />',$email);
			...
			$db -> Execute("insert into " . TABLE_ORDERS_STATUS_HISTORY . "
				...
				values ('" . zen_db_input($oID) . "', '" . zen_db_input($status) . "', now(), " . zen_db_input($customer_notified) . ", '" . zen_db_input($_POST[comments])  . "')");
			/* }
			else
			{
				...
				if( trim($ot_title) && trim($ot_value) )
				{
					...
					if($ot_total_id > 0)
					{
                     ...
                     title = '$ot_title',
						...
						$db -> Execute($Query);
						if($ot_class == "ot_total"){
							$db -> Execute("update orders set order_total= '" . $ot_value . "' where orders_id=$oID");
...
$result10a=mysql_query("SELECT options_values_price, price_prefix FROM ".TABLE_PRODUCTS_ATTRIBUTES." WHERE options_values_id='$_POST[$sendoptionon]' AND products_id='$add_product_products_id'")
				or die("Failed to connect database: ");
				while($row10a=mysql_fetch_array($result10a, MYSQL_NUM)) { $addtotheprice[$i]=$row10a[0]; $addorsubfromprice[$i]=$row10a[1];
                                        if ($row10a[1] == "-")
                                             $AddedOptionsPrice -= $row10a[0];
                                        else
					     $AddedOptionsPrice += $row10a[0]; }
}
}
...
$Query = "insert into " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " set
						...
						products_options = '" . $attributestype[$i] . "',
						products_options_values = '" . $attributes[$i] . "',
...
/* if(IsSet($add_product_options))
			...
			$db -> Execute($Query);
			$db -> Execute("update orders set order_total= '" . $Total . "' where orders_id=$oID");
//===========================================================2010=========================================
			zen_redirect(zen_href_link("edit_orders.php", zen_get_all_get_params(array('action')) . 'action=edit'));
...
?>
      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?> #<?php echo zen_get_order_no($oID); ?><?php echo zen_draw_hidden_field('update_order_no',zen_get_order_no($oID)); ?></td>
...
<!-- Begin Products Listing Block -->
	...
	<?php 
      ...
      while (!$orders_products_query -> EOF){
		$order->products[$index] = array('qty' => $orders_products_query->fields['products_quantity'],
 										'id' => $orders_products_query->fields['products_id'],
	...
	for ($i=0; $i<sizeof($order->products); $i++) {
	  ...
	  if(stripos(zen_get_products_image($order->products[$i]['id']),','))
    	{
    		$imageSrc = substr(zen_get_products_image($order->products[$i]['id']),0,strpos(zen_get_products_image($order->products[$i]['id']),','));
    	}
    	else 
    	{
    		$imageSrc = zen_get_products_image($order->products[$i]['id']);
    	}
		echo '	  <tr class="dataTableRow">' . "\n" .
		   '        <td class="' . $RowStyle . '" valign="top">' .zen_info_image($imageSrc, $order->products[$i]['name'], 30, 30) . '</td>' . "\n" .
		...
		if (sizeof($order->products[$i]['attributes']) > 0) {
			for ($j=0; $j<sizeof($order->products[$i]['attributes']); $j++) { 
				...
				echo '<br><nobr><small>&nbsp;<i> Option for Product &nbsp;&nbsp;' . "<input name='update_products[$orders_products_id][attributes][$orders_products_attributes_id][option]' size='6' value='" . $order->products[$i]['attributes'][$j]['option'] . "'>" . ': ' . "<input name='update_products[$orders_products_id][attributes][$orders_products_attributes_id][value]' size='10' value='" . $order->products[$i]['attributes'][$j]['value'] . "'>";
                                echo '</i></small></nobr>';
				echo '<br><nobr><small>&nbsp;<i> Option Price Change ' . "<input name='update_products[$orders_products_id][attributes][$orders_products_attributes_id][prefix]' size='6' value='" . $order->products[$i]['attributes'][$j]['prefix'] . "'>" . ' ' . "<input name='update_products[$orders_products_id][attributes][$orders_products_attributes_id][price]' size='10' value='" . $order->products[$i]['attributes'][$j]['price'] . "'>";
				echo '</i></small></nobr>';
			}
		  } 
...
<?php
	...
	for ($i=0; $i<sizeof($order->totals); $i++) {
		$TotalsArray[] = array("Name" => $order->totals[$i]['title'], "Price" =>$order->totals[$i]['value'], "Class" => $order->totals[$i]['class'], "TotalID" => $order->totals[$i]['orders_total_id']);
	...
	foreach($TotalsArray as $TotalIndex => $TotalDetails)
	{
		...
		else
		{
		  ...
		  {$str = "";}
			echo	'	      <tr>' . "\n" .
				'		<td align="right" class="' . $TotalStyle . '">' . "<input name='update_totals[$TotalIndex][title]' size='" . strlen(trim($TotalDetails["Name"])) . "' value='" . trim($TotalDetails["Name"]) . "'>" . '</td>' . "\n" .
				'		<td align="right" class="' . $TotalStyle . '">' . "<input name='update_totals[$TotalIndex][value]' size='6' value='" . $str . $TotalDetails["Price"] . "'>" .
