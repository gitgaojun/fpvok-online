 ...
 * @version $Id: orders.php 5452 2007-12-14 14:58:57Z ropu - added google checkout code$
 */
...
//          $update_downloads_query = "update " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_maxdays='0', download_count='0' where orders_id='" . $_GET['oID'] . "' and orders_products_download_id='" . $_GET['download_reset_off'] . "'";
      ...
      case 'update_order':
		...
		$orders_t = $db->Execute("select order_no from " . TABLE_ORDERS . "
                            where orders_id = '" . (int)$oID . "'");
		$oIDname = $orders_t->fields['order_no'];
...
// ** GOOGLE CHECKOUT **
  ...
  //send emails, not a google order or configured to use both messaging systems
	            zen_mail($check_status->fields['customers_name'], $check_status->fields['customers_email_address'], EMAIL_TEXT_SUBJECT . ' #' . $oIDname, $message, STORE_NAME, EMAIL_FROM, $html_msg, 'order_status');				
	            $customer_notified = '1';
	//send extra emails
            }
            if (SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO_STATUS == '1' and SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO != '') {
              zen_mail('', SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO, SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . ' #' . $oIDname, $message, STORE_NAME, EMAIL_FROM, $html_msg, 'order_status_extra');
...
?>
      ...
      <tr>
        <td class="main"><strong><?php echo ENTRY_ORDER_ID .$oID. zen_get_order_no($oID); ?></strong></td>
      </tr>
      <tr>
...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      ...
      echo '          <tr class="dataTableRow">' . "\n" .
           '            <td class="dataTableContent" valign="top">' .zen_info_image($imageSrc, $order->products[$i]['name'], 30, 30) . '</td>' . "\n" .
           '            <td class="dataTableContent" valign="top" align="right" style="font-weight:bold;">' . $order->products[$i]['qty'] . '&nbsp;x</td>' . "\n" .
           '            <td class="dataTableContent" valign="top"><a href="' . zen_catalog_href_link(zen_get_info_page($order->products[$i]['id']), 'products_id=' . (int)$order->products[$i]['id']) . '" class="ih" target="_blank">' . $order->products[$i]['name'].'</a>';
...
?>
                <td class="dataTableHeadingContent" align="center" width="80"><?php echo TABLE_HEADING_ORDERS_ID; ?></td>
                <td class="dataTableHeadingContent" align="left" width="100"><?php echo TABLE_HEADING_PAYMENT_METHOD; ?></td>
...
?>
                <td class="dataTableContent" align="right"><?php echo $show_difference . zen_get_order_no($orders->fields['orders_id']); ?></td>
