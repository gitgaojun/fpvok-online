...
define ('ERROR_INI_MULTIPLE_IC_LOADER_LINES',208);
...
function remote_file_contents($url)
{
    ...
    if ($remote_file_opening) {
        ...
        if ($fh) {
            ...
            stream_set_timeout($fh,IC_CONNECT_TIMEOUT);
            while (!feof($fh)) {
            ...
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT,IC_CONNECT_TIMEOUT);
            $output = curl_exec($ch);
...
function zend_extension_instructions($server_type,$loader_dir)
{
    ...
    if (SERVER_SHARED == $server_type && own_php_ini_possible()) {
        ...
        $ini_path = $html_dir . DIRECTORY_SEPARATOR . $php_ini_name;
        if (file_exists($ini_path)) {
            ...
            $edit_line = "<li>Edit the <code>$php_ini_name</code> in your <code>$html_dir</code> directory";
            if ($missing_ze_line && is_writeable($ini_path) && is_writeable($html_dir)) {
            ...
            $download_ini_file = "<li><a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">Save this  <code>$php_ini_name</code> file</a> and upload it to your html directory, <code>$html_dir</code>";
        ...
        if (empty($sysinfo['PHP_INI_DIR'])) {
            ...
            echo "<li><a href=\"$base&amp;page=phpconfig&amp;newlinesonly=1&amp;ininame=$php_ini_name\">Save this $php_ini_name file</a> and put it in your ini files directory, <code>${sysinfo['PHP_INI_DIR']}</code>";
            $editing_ini = false;
        }
    } else {
    ...
    if ($editing_ini) {
        ...
        if (isset($sysinfo['PHP_INI']) && file_exists($sysinfo['PHP_INI'])) {
            ...
            $missing_ze_line = is_array($loader_loc) && array_key_exists(ERROR_INI_ZE_LINE_NOT_FOUND,$loader_loc);
            if ($missing_ze_line) {
                echo "<a>Alternatively, replace your current <code>${sysinfo['PHP_INI']}</code> file with <a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">this new $php_ini_name file</a>."; 
            }
        }
    }
    echo '</li>';
...
function list_loader_errors($errors = array(),$warnings = array(),$suggest_restart = true)
{
    ...
    if (!empty($errors)) {
        ...
        } elseif (SERVER_SHARED == find_server_type()) {
...
function phpconfig_page()
{
    ...
    if (empty($exclude_original) && isset($sys['PHP_INI']) && file_exists($sys['PHP_INI'])) {
...
function extra_page()
{
    ...
    if (possibly_se_linux()) {
        $lines["OS extra security"] = "SELinux";
    } else {
        $lines["OS extra security"] = "None";
    }
...
function footer($update_info = null)
{
    ...
    $loader_wizard_loc = LOADERS_PAGE;
